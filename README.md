...
Video retrieval systems are temporally blind: reversed videos return near-identical embeddings, and no downstream signal flags the failure. Bag-of-frames matches by construction; bag-of-tokens yields cosine s_rev = 0.984. This blindness is well-documented for generative QA but uncharacterized for *retrieval embeddings*, where failures are silent. We diagnose temporal sensitivity across seven retrieval benchmarks spanning copy detection, temporal manipulation, and high-overlap motion retrieval. A feature-vs-comparator decomposition shows that, within a given encoder, the matching stage (per-frame representation and sequence-aware similarity) governs temporal sensitivity: on Honda HDD, switching from pooled-cosine to per-frame DTW explains 89% of the gap between bag-of-tokens and the temporal residual (single-dataset point estimate; cluster-level block-bootstrap CIs 7–32× wider than pair-level), and a non-DTW set-matching aggregation already closes 76%. On nuScenes the split inverts: encoder-sequence DTW (0.867) exceeds the residual (0.815, marginally overlapping CIs), so matching alone accounts for the full BoT→best-method gap there. Direction replicates across datasets; magnitude does not. A two-stage BoT→DTW rerank validates end-to-end: at k≈1000 on HDD (~60% of the corpus), survivor-AP reaches 0.934, within 0.008 of the full-DTW ceiling. At smaller k, the BoT pre-filter's recall (0.35 at k=100) is binding, not the reranker, exposing motion-aware first-stage filtering as a concrete open problem. A *temporal scramble gradient* (K ∈ {1, …, 16}) cleanly separates order-invariant from sequence-aware methods. Embedding probes across three open-weight Video-LLMs localize where temporal signal is lost: temporal-derivative DTW separates forward from reverse perfectly (an algebraic property of derivatives under reversal, not of the features themselves), but mean-pooling erases this signal (s_rev ≈ 1.0). Generative probes on Claude Opus 4.6 and Gemini 3.1 Pro remain near chance, with Gemini at 0.599 — marginally above chance but far from reliable. Neither linear nor nonlinear probes recover temporal order from LLM hidden states (126 configurations, though the p ≫ n regime bounds recoverability rather than proving absence). No single method spans both copy detection and motion retrieval, exposing a sensitivity–invariance trade-off that current architectures do not resolve. We release open-source code packaging the scramble gradient and reversal tests.
