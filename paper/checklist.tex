...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract distinguishes exact invariance of fixed independently encoded elements from empirical behavior of contextual representations, identifies pair-classification results as diagnostics, and avoids causal attribution. It also reports that the corrected global HDD retrieval result reverses the conditional pair-diagnostic ranking.
...
\item {\bf Theory assumptions and proofs}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The code, frozen-model revisions, dataset versions, pair construction, evaluation parameters, corrected SLURM jobs, compact result summaries, and artifact provenance are supplied. Feature caches and licensed datasets are not redistributed, but the guide documents how to regenerate the caches from the specified source data and model revisions.
...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerNo{}
    \item[] Justification: Pair-resampling intervals are labeled descriptive because pairs share clips. The main HDD and nuScenes method comparisons use paired intersection-cluster bootstrap contrasts, and the VCDB scramble uses variation over 10 seeds. Analogous independent-unit analyses are not available for VCDB pair AP, Nymeria, MUVR, or SSv2, so the paper avoids inferential claims for those comparisons. Prompt standard deviations measure prompt sensitivity, not sampling uncertainty.
