...
\textbf{Order signal is recoverable from per-frame features but erased by pooling.} Pooled VLM vision embeddings are order-invariant ($s_\text{rev} \approx 1.0$). Retaining per-frame tokens and comparing via temporal derivative DTW achieves perfect forward/reverse separation (balanced accuracy\,=\,1.000) for both CLIP and SigLIP~\cite{weller2025limits}. This follows from the mathematical structure of temporal derivatives under DTW (reversed derivatives are negated), not learned understanding. Qwen3-VL's pooled vision embeddings underperform sharply (VCDB AP\,=\,0.491 vs.\ 0.915--0.926), possibly because its 3D temporal--spatial merging entangles frame identity with spatial position, a hypothesis consistent with the 3D-merging architectural class, though we do not isolate this mechanistically.

\textbf{Where does the signal go?} The LLM backbone does not surface it under any pooled readout ($s_\text{rev} \approx 1.0$ across all layers; Appendix~\ref{sec:layer_ablation}); logistic-regression and 2-layer MLP probes on per-position hidden states likewise fail (best 0.560 across 126 configurations; Appendix~\ref{sec:linear_probe}). This $p \gg n$ regime ($D \approx 4{,}000$, $N = 400$) bounds \emph{recoverability}, not absence: full-sequence DTW on the same per-position states separates forward from reverse cleanly (Appendix~\ref{sec:vision_token_probe}). Symmetric aggregation, not the backbone, is the bottleneck. \textbf{Practical consequence:} VLM APIs that expose only pooled embeddings cannot support order-sensitive retrieval without a second-stage per-frame DTW or a custom pooled readout.
...
\label{alg:decision}
\begin{algorithmic}[1]
\State Run \emph{scramble gradient}. Flat AP across $K$ $\Rightarrow$ class A/A$'$ (architectural order-invariance); \textbf{stop}.
\State If AP degrades with $K$, run \emph{feature-vs-comparator decomposition}: attribute gap to encoder (class B) or matching stage (class C).
\State Run \emph{sampling-density sweep} (context window $\times$ frame rate); isolate class D/D$'$ (frame-rate sensitivity).
\State Surface classes E/E$'$ (evaluation-protocol) and F/F$'$ (behavioral) via cluster-level CIs and generative-vs-embedding probe disagreement.
\end{algorithmic}
\end{algorithm}
...
No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}): within a given encoder, the matching stage (not the encoder) explains 89\% of the HDD gap, and pooling erases per-frame order across every VLM layer we probed. These failures form a ten-class taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by Algorithm~\ref{alg:decision}; the open problem is designing indexable, order-sensitive comparators that preserve what current aggregators discard.%
\ifdefined\neuripssubmission
\else
