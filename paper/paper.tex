...
\textbf{Limitations.} All evaluations use pairwise similarity; DTW is not indexable ($O(T^2)$); probes train on 400 samples in a $p \gg n$ regime ($D \approx 4{,}000$), so the null bounds recoverability at this sample size and dimensionality rather than proving absence of temporal signal; ViCLIP's 8-frame input confounds its comparison with 64-frame V-JEPA~2 (the context sweep at equivalent 8-frame density suggests a ${\sim}0.20$ AP drop; Section~\ref{sec:context_sweep}). Temporal manipulation is one axis of the copy-detection attack surface; spatial occlusion (overlays, watermarks, color-channel perturbations) is a separate and often more prevalent threat, orthogonal to the blindness we diagnose.

\textbf{Practitioner recommendations.} (1)~Test your pipeline with the scramble gradient at $K \in \{1, 4, 16\}$: flat AP across $K$ means the system is temporally blind. (2)~For copy detection, match the method to the action cost: high-precision actions (takedowns, bans) favor BoF with a conservative threshold; soft actions (ranking demotions, review queues) can accept temporal comparators that flag reversal. (3)~For motion-sensitive retrieval, a two-stage BoF pre-filter $\to$ DTW rerank on top-$k$ is practical at ${\sim}100$\,ms overhead per query (Appendix~\ref{sec:computational_costs}).

\section{Conclusion}

Across six benchmarks, no single method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}). The temporal scramble gradient cleanly separates method classes; the feature-vs-comparator decomposition places 89\% of the HDD gap in the matching stage, and on nuScenes the matching stage alone is on par with the learned residual (AP\,=\,0.867 vs.\ 0.815, marginally overlapping CIs). Training-time fixes (TARA, PL-Stitch) do not close the gap on HDD: under the backbones evaluated here, within a given encoder, per-frame matching, not the training objective, governs retrieval discrimination. VLM embedding probes show pooling erases the per-frame order signal (126 configurations at chance). Indexable order-sensitive comparators---with learnable pooling and query-adaptive routing---remain the key open problem.\footnote{%
\ifdefined\neuripssubmission
