...
temporal order from LLM hidden states (126 configurations). No method spans both
copy detection and motion retrieval, exposing a sensitivity--invariance trade-off
that current architectures do not resolve.
\end{abstract}
...
All three open-weight VLMs perform at chance. Claude~4.6 Opus~\cite{claude4opus} (200 sequences, 16 frames via API) achieves $0.545 \pm 0.055$, no better. Gemini~3.1 Pro~\cite{gemini31pro}, a reasoning model that expends thinking tokens before answering, reaches $0.599 \pm 0.010$---marginally above chance but far from reliable temporal perception. Answers across all five models are driven by prompt phrasing, not temporal direction (Appendix~\ref{sec:integrity_probe} and~\ref{sec:prompt_details}).
