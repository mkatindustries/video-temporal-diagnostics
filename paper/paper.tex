...
\textbf{Evaluation protocols.} We report average precision (AP; area under the precision--recall curve) and, where applicable, area under the ROC curve (AUC) as retrieval metrics. All evaluations are pairwise. \emph{VCDB}: 528 videos; 5{,}585 positive pairs from annotations, negatives sampled 1:1 with fixed seed (sensitivity analysis across 1:1, 1:5, and all-pairs in Appendix~\ref{sec:neg_sampling}). \emph{HDD}: all $\binom{n}{2}$ pairs within each GPS-clustered intersection (DBSCAN, $\epsilon\!\approx\!30$\,m); positive = same maneuver; 50 mixed clusters, 1{,}687 maneuvers. \emph{nuScenes}: same protocol on ego\_pose; CAN bus--derived labels; 264 segments, 950 pairs. \emph{EPIC s\textsubscript{rev}}: per-sequence self-comparison on 500 sequences; bootstrap 95\% CIs (1{,}000 resamples). \emph{Nymeria}: 100 egocentric sessions, 8{,}780 segments; positive = same activity same room; all $\binom{n}{2}$ pairs (394{,}210). \emph{MUVR News}: 9{,}958 videos, 74 topics, 27{,}453 positive pairs.
...
\textbf{Practitioner recommendations.} (1)~Copy detection: high-precision actions favor BoF; soft actions accept temporal comparators. (2)~Motion retrieval: BoT$\to$DTW rerank recovers 99\% of full-DTW AP at $k\!\approx\!1000$ on HDD ($\sim$40\% compute saving); at small $k$, pre-filter recall is binding (Table~\ref{tab:rerank_hdd}).

\vspace{-1em}
\section{Conclusion}
\vspace{-0.6em}

No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}): within a given encoder, the matching stage (not the encoder) explains 89\% of the HDD gap, and pooling erases per-frame order across every VLM layer we probed. A two-stage BoT$\to$DTW reranker confirms the decomposition end-to-end but exposes an asymmetry: appearance-based pre-filters recover only 35\% of motion-retrieval positives at $k\!=\!100$ (Table~\ref{tab:rerank_hdd}), so the pairwise gap does not compose into a cheap retriever. These failures form a ten-class taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by Algorithm~\ref{alg:decision}. The open problem is constructive: designing indexable, order-sensitive retrieval pipelines---including motion-aware pre-filters---that preserve the per-frame signal current aggregators discard; world model training objectives~\cite{lecun_worldmodel,vjepa2} offer the most promising path.%
\ifdefined\neuripssubmission
\else
