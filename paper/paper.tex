...
embeddings (the numerical fingerprints used for similarity search)---bag-of-frames is
identical by construction, bag-of-tokens yields cosine $s_\text{rev}\!=\!0.984$---and no
downstream signal flags the failure. This blindness is well-documented for
generative question answering but uncharacterized for \emph{retrieval
embeddings}, where failures are silent. We diagnose temporal sensitivity
across six benchmarks. A feature-vs-comparator decomposition shows that,
within a given encoder, the matching stage---not the encoder---governs
...
\begin{figure}[h]
    ...
    \caption{VCDB Reversal Attack (528 videos, 5{,}585 positive pairs). Order-invariant methods (BoF, Chamfer) maintain identical AP/AUC under reversal. Only sequence-aware methods (Attention Trajectory) register the structural tampering via an AP drop. Bar-chart visualization of the main-body reversal table.}
...
\begin{figure}[h]
    ...
    \caption{Honda HDD maneuver discrimination (1{,}687 maneuvers, 128 sessions). V-JEPA~2 temporal residuals achieve near-perfect separation (AP\,=\,0.956) while DINOv3 methods remain at chance. Bar-chart visualization of the main-body HDD maneuver discrimination table.}
