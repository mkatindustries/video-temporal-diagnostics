...
per-frame dynamic time warping descriptively closes 89\% of the pair-classification AP gap between a
V-JEPA~2 bag-of-tokens baseline and its temporal residual; nuScenes supports the same
qualitative conclusion, while Something-Something V2 favors a different representation
and comparator. VLM results are readout- and prompt-dependent: sequence comparisons expose
...
\begin{figure}[ht]
    \centering
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{1.2em}
    Corrected balanced-chunk scramble results pending cluster rerun.
    \vspace{1.2em}}}
    \caption{Temporal Scramble Gradient on VCDB. This figure will be regenerated with near-equal chunk partitions; the original implementation produced an oversized tail chunk when $T$ was not divisible by $K$ (Appendix~\ref{sec:scramble_data}).}
    \label{fig:scramble_gradient}
\end{figure}

The corrected implementation uses near-equal chunks and reports multi-seed uncertainty. We defer numerical interpretation until that run is complete; the previous single-seed curve used an unbalanced tail partition and is not evidence for a quantitative ranking.
...
Encoder-sequence DTW reaches AP\,=\,0.942. Descriptively, the switch from pooled cosine to per-frame DTW closes 89\% of the observed BoT-to-residual AP gap (0.825$\to$0.956). Cluster-level CIs are 7--32$\times$ wider than pair-level intervals (Appendix~\ref{sec:bootstrap_cis}); formal method comparisons require a paired cluster bootstrap, which we report with the corrected run. Symmetric-max aggregation~\cite{kordopatis2019visil} on the same encoder-sequence patches reaches AP\,=\,0.914. Comparing these observed point estimates, Chamfer closes 76\% of the BoT$\to$encoder-sequence-DTW gap; this contrast does not uniquely attribute the remainder to DTW's monotonicity because the comparators differ in more than that constraint.

\textbf{Diagnostic-protocol stress test: does the decomposition compose into a fast retriever?} We evaluate an indexable BoT first stage followed by encoder-sequence-DTW reranking. The corrected protocol is directed and query-wise: every cached HDD segment is a gallery candidate; relevance requires the same GPS cluster and maneuver label; candidates outside the query's cluster remain negatives; and truncated AP@$k$ is normalized by the query's total number of relevant items, so it is bounded by recall@$k$. The previous unordered-pair experiment admitted a pair when either endpoint survived and omitted out-of-cluster negatives, so its AP values are withdrawn. Corrected query-macro AP, recall, and MRR are pending the cluster run (Appendix~\ref{sec:rerank_appendix}).

\begin{table}[ht]
\centering
\caption{Corrected directed BoT$\to$DTW reranking protocol on HDD. Numerical results are pending rerun.}
\label{tab:rerank_hdd}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{macro AP} & \textbf{recall@$k$} & \textbf{MRR} \\
\midrule
V-JEPA~2 BoT only & \multicolumn{3}{c}{\emph{pending corrected run}} \\
\; + DTW rerank, $k\!\in\!\{10,25,50,100,250\}$ & \multicolumn{3}{c}{\emph{pending corrected run}} \\
V-JEPA~2 Encoder-Seq DTW (full) & \multicolumn{3}{c}{\emph{pending corrected run}} \\
\bottomrule
\end{tabular}
\end{table}
...
The point-estimate ranking is directionally consistent with a comparator effect (Table~\ref{tab:summary_encoders}). V-JEPA~2 encoder-sequence DTW reaches AP\,=\,0.867, ahead of the temporal residual (0.815) and BoT (0.791), while DINOv3 methods are at $\leq$0.623. The reported marginal intervals overlap for the top two ([0.842,\,0.890] vs.\ [0.785,\,0.844]; Appendix~\ref{sec:bootstrap_cis}) and do not test paired method differences. We therefore treat both the residual--DTW ordering and the BoT contrast as descriptive until paired cluster-bootstrap results are available. Differences in native sampling are one possible explanation, not an identified cause.
...
\quad BoT & 0.984 \\
\quad Temporal Residual & \emph{rerun with sequence DTW} \\
\midrule
...
\textbf{Motion dynamics} (HDD, nuScenes, SSv2). The best observed representation--comparator combination is domain-dependent: V-JEPA~2 variants have the strongest HDD and nuScenes point estimates, while attention trajectories lead on the selected SSv2 split. Temporal-training controls (TARA~\cite{tara}, PL-Stitch~\cite{plstitch}, ViCLIP~\cite{viclip}, X-CLIP~\cite{xclip}) do not lead these particular evaluations, but the evidence does not support a universal comparator ranking.
...
\textbf{Architecture implications.} Symmetric aggregation over independently encoded elements guarantees invariance; contextual encoding can retain temporal effects before pooling. Indexable order-sensitive retrieval may therefore require permutation-sensitive pooling, trajectory tokenization, or motion-aware first-stage descriptors (Section~\ref{sec:motion}).
...
We introduced executable diagnostics for locating temporal sensitivity in video-retrieval pipelines. Exact invariance is confined to symmetric comparisons of fixed element representations; contextual encoders, comparator choice, prompt, and readout all affect the empirical outcome. The strongest observed representation--comparator combination is domain-dependent, and the current pair-classification results do not automatically imply an effective end-to-end retriever. A corrected directed reranking evaluation and paired group-level uncertainty estimates are therefore part of the required validation.\footnote{Toolkit code: \url{https://anonymous.4open.science/r/video-temporal-diagnostics-F450}.}
...
Table~\ref{tab:context_sweep} and Figure~\ref{fig:context_sweep} report the Section~\ref{sec:context_sweep} context experiment. The parameter $c \in \{1.0, 2.0, 3.0, 4.0, 6.5, 8.0\}$ is seconds of padding on each side of a variable-duration maneuver; it is not the final clip duration. We extract 64 unique frames from each resulting window.

\begin{figure}[h]
    \centering
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{1.2em}
    Context plot pending regeneration with per-side padding on the horizontal
    axis and without the invalid $64/c$ effective-FPS axis.
    \vspace{1.2em}}}
    \caption{V-JEPA~2 context-padding sweep on Honda HDD; numerical values remain in Table~\ref{tab:context_sweep}.}
...
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{1.2em}
    Replacement visualization will separate cosine and DTW scales; the former
    shared-axis chart is withdrawn.
    \vspace{1.2em}}}
    \caption{Comparator-specific $s_\text{rev}$ visualization pending corrected V-JEPA~2 residual run.}
...
V-JEPA 2 Temporal Res. & 0.698 & \textbf{0.956} & 0.815 & 0.556 & \emph{rerun} \\
...
\begin{flushleft}
\small --- = not evaluated. VCDB AP depends on negative seed ($\pm$0.06; Appendix~\ref{sec:neg_sampling}). $s_\text{rev}$ values from cosine and DTW are not cross-comparable; the temporal-residual value is pending rerun after replacing flattened cosine with sequence DTW.
\end{flushleft}
\end{table}
...
The original experiment ranked unordered in-cluster pairs, admitted a pair when either endpoint survived, omitted out-of-cluster gallery negatives, and normalized AP by retrieved positives. Those choices can produce AP above recall and do not represent per-query retrieval. We withdraw that table.

\begin{table}[h]
\centering
\caption{Corrected directed, query-wise HDD reranking evaluation. Results are pending rerun.}
\label{tab:rerank_full}
\begin{tabular}{rccc}
\toprule
$k$ & macro AP@$k$ & recall@$k$ & MRR \\
\midrule
10, 25, 50, 100, 250 & \multicolumn{3}{c}{\emph{pending corrected run}} \\
\bottomrule
\end{tabular}
\end{table}

For each query, the corrected gallery contains every other cached segment. Relevance requires both the same GPS cluster and the same maneuver label; all other segments are negatives. The BoT ranking selects directional top-$k$ candidates, DTW reranks only those candidates, and AP@$k$ divides by the total relevant count for the query. Metrics are averaged over eligible queries, with intersection-cluster bootstrap intervals. The implementation is \texttt{experiments/eval\_hdd\_bof\_dtw\_rerank.py}.
...
\begin{figure}[h]
    ...
    \caption{nuScenes pair diagnostic (264 segments, 950 pairs, 50 intersection clusters). V-JEPA~2 encoder-sequence DTW has the largest point estimate (AP\,=\,0.867), followed by the temporal residual (0.815); a paired cluster-bootstrap contrast is pending.}
...
The original single-seed and raw-frame scripts formed $K-1$ short chunks plus one oversized tail when $T$ was not divisible by $K$ (for example, $T\!=\!30,K\!=\!16$ produced fifteen one-frame chunks and one fifteen-frame chunk). The corrected implementation uses near-equal partitions from \texttt{tensor\_split}/\texttt{array\_split}. Table~\ref{tab:scramble_data} is intentionally left pending until the corrected multi-seed and raw-frame runs complete.

\begin{table}[h]
\centering
\caption{Corrected AP across near-equal scramble partitions on VCDB. Results are pending rerun.}
\label{tab:scramble_data}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lc}
\toprule
\textbf{Method} & \textbf{Corrected multi-seed gradient} \\
\midrule
All methods & \emph{pending corrected run} \\
\bottomrule
\end{tabular}
}
\end{table}

The extracted-embedding gradient tests the comparator's response while keeping token values fixed. The raw-frame variant re-encodes each shuffled video and therefore measures the combined encoder and comparator response. We will report both, with identical partition semantics and seeds, rather than attributing non-monotonicity before the corrected run.

\begin{figure}[h]
    \centering
    \fbox{\parbox{0.82\textwidth}{\centering\vspace{1.2em}
    Corrected multi-seed balanced-chunk results pending cluster rerun.
    \vspace{1.2em}}}
    \caption{Corrected multi-seed scramble gradient placeholder; the figure will be regenerated from near-equal chunk partitions.}
...
\textbf{Reproduction recipe.} Experiment entry points and required caches are documented in \texttt{REPRODUCIBILITY.md}. The repository does not yet provide a one-command reproduction target or committed feature caches, so runtime depends on local data access and model downloads. The corrected cluster jobs supplied with the code regenerate the invalidated scramble, EPIC residual, and directed HDD reranking results before submission.
