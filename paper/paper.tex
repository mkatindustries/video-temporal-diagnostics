...
bag-of-tokens yields cosine $s_\text{rev}\!=\!0.984$. This blindness is
...
The LLM backbone does not surface this signal under any pooled readout ($s_\text{rev} \approx 1.0$ across all layers; Appendix~\ref{sec:layer_ablation}). Neither logistic regression nor 2-layer MLP probes on per-position LLM hidden states recover temporal order above chance (best: 0.560; all 126 configurations within chance range; Appendix~\ref{sec:linear_probe}). The $p \gg n$ regime ($D \approx 4{,}000$, $N = 400$) bounds \emph{recoverability} rather than proving absence; the per-position DTW result below shows the signal is not absent, only not linearly separable in fixed-dimensional reductions. Yet per-position hidden states \emph{do} retain order signal under full-sequence DTW (Appendix~\ref{sec:vision_token_probe}). Symmetric aggregation, not the LLM backbone itself, is the bottleneck. In practice, VLM providers who expose only pooled embeddings cannot support order-sensitive retrieval out of the box: a second-stage per-frame DTW or a custom pooled readout is required.
...
\State Run \emph{sampling-density sweep} (context window $\times$ frame rate); isolate class D/D$'$.
...
Table~\ref{tab:failure_taxonomy} is thus the classifier, not a post-hoc label.
...
No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}): within a given encoder, the matching stage (not the encoder) explains 89\% of the HDD gap (nuScenes corroborates direction, not magnitude; Section~\ref{sec:discussion}), and pooling erases per-frame order across every VLM layer we probed. These failures compose into a ten-class taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by an executable protocol (Algorithm~\ref{alg:decision}); the open problem is constructive: designing indexable, order-sensitive comparators that preserve the per-frame signal current aggregators discard.%
\ifdefined\neuripssubmission
\else
