...
\textbf{Practitioner recommendations.} (1)~Test with the scramble gradient at $K\!\in\!\{1,4,16\}$: flat AP $\Rightarrow$ temporally blind. (2)~For copy detection, high-precision actions (takedowns, bans) favor BoF; soft actions (ranking demotions, review queues) can accept temporal comparators. (3)~For motion retrieval, BoF pre-filter $\to$ top-$k$ DTW rerank adds ${\sim}100$\,ms/query (Appendix~\ref{sec:computational_costs}).

\section{Conclusion}

No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}): within an encoder, the matching stage explains 89\% of the HDD gap; training-time fixes do not close it. Pooling erases per-frame order signal across VLM layers. Indexable order-sensitive comparators with learnable pooling remain the key open problem.\footnote{%
\ifdefined\neuripssubmission
