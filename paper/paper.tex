...
A feature-vs-comparator decomposition reveals that, within a given encoder,
the matching stage (per-frame representation and sequence-aware similarity)
governs temporal sensitivity: on Honda HDD this switch explains 89\% of the
performance gap between bag-of-tokens and the temporal residual (point estimate;
cluster-level block-bootstrap CIs are wider), with a non-DTW set-matching
aggregation already closing 76\%; on nuScenes the matching stage alone is on
par with the learned residual transform (AP\,=\,0.867 vs.\ 0.815; CIs
marginally overlap). A \emph{temporal scramble gradient}
($K \in \{1, \ldots, 16\}$) cleanly separates order-invariant from
sequence-aware methods. Embedding probes across three open-weight Video-LLMs
localize where temporal signal is lost: temporal-derivative DTW separates
forward from reverse perfectly---an algebraic property of derivatives under
reversal, not of the features themselves---but mean-pooling erases this signal
($s_\text{rev} \approx 1.0$). Generative probes on Claude~4.6 Opus and
Gemini~3.1 Pro fare no better than the open-weight models. Neither linear nor
nonlinear probes recover temporal order from LLM hidden states
(126 configurations, though the $p \gg n$ regime bounds recoverability rather
than proving absence). Across all evaluated methods, no single approach spans
both copy detection and motion retrieval, exposing a sensitivity--invariance
...
Modern video retrieval represents videos as mean-pooled frame embeddings from pretrained encoders. This \emph{bag-of-frames} (BoF) approach excels at Video Copy Detection (VCD)---identifying re-encoded, cropped, or camcorded copies---because visual semantics survive frame reordering, enabling billion-scale nearest-neighbor search~\cite{faiss, semdedup}.

BoF discards temporal order entirely. A growing literature documents this ``temporal blindness'' in Video-LLMs~\cite{tomato,temporalbench,tempcompass,infact,videozerobench,vbenchcomp}: generative probes show near-chance temporal QA; frame shuffling leaves benchmark scores intact. These diagnoses target generative QA, where failures surface in text. In retrieval, failures are silent---reversed videos return identical embeddings, left turns match right turns---and no downstream signal flags them. We fill this gap with three test cases:

\textbf{1. Copy Detection \& Scene Retrieval:} when the discriminative signal is visual background (VCD; activities in different rooms), semantic pooling suffices.

\textbf{2. Temporal Manipulation (Trust \& Safety):} reversed video yields BoF similarity of 1.0, evading embedding-based copy retrieval~\cite{fojcik2025}.

\textbf{3. High-Overlap Motion Retrieval (Dashcam Fleets):} querying a fleet for ``left turns at Intersection X'' requires encoding motion dynamics; static background dominates the BoF embedding, conflating left and right turns.
...
\textbf{Symmetric aggregation implies permutation-invariance.}
Mean-pooling, max-pooling, and Chamfer matching are symmetric aggregators: they satisfy $g(f_1, \ldots, f_T) = g(f_{\pi(1)}, \ldots, f_{\pi(T)})$ for all permutations $\pi$~\cite{deepsets}. BoF order-invariance therefore follows from the aggregation operator, independent of encoder capacity or training objective.
...
\begin{table}[ht]
\centering
\caption{VLM embedding probes across all tasks, grouped by pipeline stage. Generative forward/reverse classification is reported separately in Section~\ref{sec:vlm_generative} (Table~3).}
...
\textbf{Limitations.} DTW is not indexable ($O(T^2)$); probes train on 400 samples in $p \gg n$ ($D \approx 4{,}000$), bounding recoverability rather than proving absence; ViCLIP's 8-frame input confounds comparison with 64-frame V-JEPA~2 (${\sim}0.20$ AP drop at equivalent density, Section~\ref{sec:context_sweep}). Spatial occlusion attacks (overlays, watermarks, color perturbations) remain a separate, often more prevalent threat orthogonal to temporal blindness.

\textbf{Practitioner recommendations.} (1)~Run the scramble gradient at $K\!\in\!\{1,4,16\}$: flat AP $\Rightarrow$ blind. (2)~Copy detection: high-precision actions favor BoF; soft actions accept temporal comparators. (3)~Motion retrieval: BoF $\to$ top-$k$ DTW rerank adds ${\sim}100$\,ms/query (Appendix~\ref{sec:computational_costs}).

\vspace{-0.5em}
\section{Conclusion}
\vspace{-0.3em}

No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}). Within an encoder, the matching stage explains 89\% of the HDD gap; pooling erases per-frame order across VLM layers. Indexable order-sensitive comparators remain the open problem. %
\ifdefined\neuripssubmission
Code and data: supplementary; see \texttt{REPRODUCIBILITY.md}.%
\else
Code: \url{https://github.com/mkatindustries/video-temporal-diagnostics}.%
\fi

\FloatBarrier  % Force all main-body floats before references

\begin{thebibliography}{99}
\bibitem{vjepa2} M.~Assran et al. {V-JEPA~2: Self-Supervised Video Models Enable Understanding of Complex Tasks}. \emph{arXiv:2506.09985}, 2025.
\bibitem{vjepa21} L.~Mur-Labadia et al. {V-JEPA~2.1}. \emph{arXiv:2603.14482}, 2026.
\bibitem{dinov3} O.~Sim\'{e}oni et al. {DINOv3: Towards Visual Foundation Models with Self-Supervised Learning}. \emph{arXiv:2508.10104}, 2025.
...
\bibitem{3dcsl} R.~Deng et al. {3D-CSL: Self-supervised 3D Context Similarity Learning for Near-Duplicate Video Retrieval}. \emph{arXiv:2211.05352}, 2022.
\bibitem{plstitch} C.~Che et al. {PL-Stitch: Learning Temporal Video Representations via Plackett-Luce Ranking}. \emph{arXiv:2511.17805}, 2025.
\bibitem{arrowgev} F.~Yu et al. {ArrowGEV: Arrow-of-Time Generalized Extreme Value Loss for Video Representation Learning}. \emph{arXiv:2601.06559}, 2026.
\bibitem{weavetime} Y.~Zhang et al. {WeaveTime: Weaving Temporal Representations for Video Understanding}. \emph{arXiv:2602.22142}, 2026.
...
\bibitem{aot} S.~Matta et al. AoT-PsyPhyBENCH: Benchmarking VLMs on arrow-of-time and psychophysical tasks. \emph{arXiv:2510.26241}, 2025.
...
\bibitem{muvr} Y.~Feng et al. Multi-Modal Untrimmed Video Retrieval. \emph{arXiv:2510.21406}, 2025.
...
\bibitem{nymeria} L.~Ma et al. Nymeria: A Massive Collection of Multimodal Egocentric Daily Motion in the Wild. In \emph{CVPR}, 2025.
...
\bibitem{nextgqa} J.~Xiao, A.~Yao, Z.~Li, and T.-S.~Chua. Can I Trust Your Answer? Visually Grounded Video Question Answering. In \emph{CVPR}, 2024. \emph{arXiv:2309.01327}.
...
\bibitem{viclip} Y.~Wang, Y.~Li, H.~Li et al. InternVid: A Large-scale Video-Text Dataset for Multimodal Understanding and Generation. In \emph{ICLR}, 2024.
...
The diagnostic suite's value is not individual numbers but a structured classification of \emph{why} video retrieval systems fail temporally. We categorize every failure surfaced in Sections~3 and~4 along six orthogonal axes and list a representative instance for each.
...
\begin{tabular}{llp{4.6cm}p{4.8cm}}
\toprule
...
\textbf{(F)} & Behavioral / output & Generative output obscures internal temporal signal & Gemma-4: 87\% reverse bias on prompt 0 $\to$ 73\% forward bias on prompt 2 \\
