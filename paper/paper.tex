...
A feature-vs-comparator decomposition reveals that the matching stage
(per-frame representation and sequence-aware similarity), not the encoder,
governs temporal sensitivity: on Honda HDD this switch explains 89\% of the
performance gap (a non-DTW set-matching aggregation already closes 76\%); on
nuScenes the matching stage alone is on par with the learned residual transform
(AP\,=\,0.867 vs.\ 0.815; CIs overlap). A \emph{temporal scramble gradient} ($K \in \{1, \ldots, 16\}$) cleanly
separates order-invariant from sequence-aware methods. Embedding probes across
three open-weight Video-LLMs localize where temporal signal is lost: per-frame
vision features separate forward from reverse perfectly under sequence-aware
comparators (by construction), but mean-pooling erases this signal
...
nonlinear probes recover temporal order from LLM hidden states (126 configurations). Across all evaluated
...
resolve. We release \textsc{TemporalDiag}, an open-source toolkit packaging
the scramble gradient and reversal tests.
\end{abstract}
...
TARA~\cite{tara}, a 7B MLLM trained with chiral negatives, hits AP\,=\,0.547 under cosine---indistinguishable from ViCLIP (0.542) despite ${\sim}23\times$ more parameters. PL-Stitch~\cite{plstitch} (ViT-Base, Plackett-Luce ranking) supports the same decomposition: BoF yields AP\,=\,0.478, DTW on raw embeddings 0.540---both near chance, and below DINOv3 Chamfer (0.559), likely a capacity (86M vs.\ 300M) and pretraining-scale gap rather than a comparator effect. Yet $s_\text{rev}\!=\!0.006$ under raw DTW confirms PL-Stitch \emph{does} encode frame order---the features are not discriminative enough for maneuver retrieval.
...
\begin{flushleft}
\small --- = not evaluated. VCDB AP depends on negative seed ($\pm$0.06; Appendix~\ref{sec:neg_sampling}).
\end{flushleft}
\end{table}
...
Gemma-4 (SigLIP) & 0.632 & 0.553 & 0.469 & 0.492 & \textbf{1.000} \\
LLaVA (CLIP) & 0.637 & 0.545 & 0.470 & 0.495 & \textbf{1.000} \\
Qwen3-VL (SigLIP) & 0.610 & 0.468 & 0.517 & 0.542 & \textbf{1.000} \\
\midrule
...
\begin{flushleft}
\small $^\dagger$Pooled representations are order-invariant by construction.
\end{flushleft}
\end{table}
...
\textbf{Limitations.} All evaluations use pairwise similarity; DTW is not indexable ($O(T^2)$); probes train on 400 samples in a $p \gg n$ regime ($D \approx 4{,}000$), so the null bounds recoverability at this sample size rather than proving absence; ViCLIP's 8-frame input confounds its comparison with 64-frame V-JEPA~2 (the context sweep at equivalent 8-frame density suggests a ${\sim}0.20$ AP drop; Section~\ref{sec:context_sweep}). Temporal manipulation is one axis of the copy-detection attack surface; spatial occlusion (overlays, watermarks, color-channel perturbations) is a separate and often more prevalent threat, orthogonal to the blindness we diagnose.

\textbf{Practitioner recommendations.} (1)~Test your pipeline with the scramble gradient at $K \in \{1, 4, 16\}$ using \textsc{TemporalDiag}: flat AP across $K$ means the system is temporally blind. (2)~For copy detection, match the method to the action cost: high-precision actions (takedowns, bans) favor BoF with a conservative threshold; soft actions (ranking demotions, review queues) can accept temporal comparators that flag reversal. (3)~For motion-sensitive retrieval, a two-stage BoF pre-filter $\to$ DTW rerank on top-$k$ is practical at ${\sim}100$\,ms overhead per query (Appendix~\ref{sec:computational_costs}).

\section{Conclusion}

Across six benchmarks, no single method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}). The temporal scramble gradient cleanly separates method classes; the feature-vs-comparator decomposition places 89\% of the HDD gap in the matching stage, and on nuScenes the matching stage alone is on par with the learned residual (AP\,=\,0.867 vs.\ 0.815, overlapping CIs). Training-time fixes (TARA, PL-Stitch) do not close the gap on HDD: under the backbones evaluated here, per-frame matching, not the training objective, governs retrieval discrimination. VLM embedding probes show pooling erases the per-frame order signal (126 configurations at chance). Indexable order-sensitive comparators---with learnable pooling and query-adaptive routing---remain the key open problem.\footnote{%
\ifdefined\neuripssubmission
...
\bibitem{vjepa2} M.~Assran et al. V-JEPA~2. \emph{arXiv:2506.09985}, 2025. See also V-JEPA~2.1 (\emph{arXiv:2603.14482}, 2026).
...
\bibitem{arrowgev} F.~Yu et al. ArrowGEV. \emph{arXiv:2601.06559}, 2026.
...
\begin{figure}[h]
    ...
    \caption{nuScenes cross-dataset validation (264 segments, 950 pairs, 50 intersection clusters). V-JEPA~2 encoder-sequence DTW (AP\,=\,0.867) is the strongest method; the matching-stage change alone is on par with the temporal residual (AP\,=\,0.815; CIs overlap, Appendix~\ref{sec:bootstrap_cis}).}
