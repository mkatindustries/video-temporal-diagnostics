...
\begin{abstract}
Video retrieval systems are temporally blind: reversed videos return near-identical
embeddings (the numerical fingerprints used for similarity search), and no
downstream signal flags the failure. Bag-of-frames matches by construction;
bag-of-tokens yields cosine similarity 0.984 between forward and reversed clips. This blindness is
...
sensitivity across six benchmarks. A feature-vs-comparator decomposition shows
that, within a given encoder, the matching stage (not the encoder) governs
...
V-JEPA~2 temporal residual (point estimate; cluster-level CIs 7--32$\times$ wider than pair-level; direction replicates on nuScenes). A \emph{temporal
scramble gradient} cleanly separates order-invariant from sequence-aware
methods. Probing three open-weight VLMs localizes where temporal signal
dies: per-frame vision features retain order, mean-pooling erases it, and no
pooled readout of LLM hidden states recovers it (126 configurations). Generative
probes on Claude~Opus~4.6 and Gemini~3.1 Pro fare no better than 7B open-weight
models. We consolidate these failures into a 10-class taxonomy with an
executable diagnostic protocol (Algorithm~\ref{alg:decision}). No single
approach spans both copy detection and motion retrieval, exposing a
...
Video retrieval embeddings fail silently under temporal manipulation: reversed videos return identical vectors, left turns match right turns, and no downstream signal flags the error. This is a consequence of architecture, not a bug. Modern systems represent videos via the \emph{bag-of-frames} (BoF) approach: mean-pooling (element-wise averaging) frame embeddings from pretrained encoders into one vector~\cite{faiss, semdedup}. BoF dominates Video Copy Detection (VCD; finding re-encoded, cropped, or camcorded copies) because visual semantics survive frame reordering, and order-invariance enables billion-scale nearest-neighbor search. The same property that makes BoF fast makes it blind.
...
Mean-pooling reversed frames gives the same vector as mean-pooling the original: the operator discards the indices. More generally, mean-pooling, max-pooling, and Chamfer matching are \emph{symmetric aggregators}: their output is invariant to any permutation of the inputs, $g(f_1, \ldots, f_T) = g(f_{\pi(1)}, \ldots, f_{\pi(T)})$~\cite{deepsets}. BoF (Bag-of-Frames) averages frame-level embeddings from an image encoder; BoT (Bag-of-Tokens) averages patch-level tokens from a spatiotemporal encoder. Both are symmetric aggregators, and their order-invariance follows from the aggregation operator, independent of encoder capacity or training objective. The diagnostics below distinguish systems whose blindness is architectural (the aggregator) from systems whose blindness is learned (the encoder).
...
Full cross-method comparisons across all six benchmarks are in Appendix~\ref{sec:cross_method_tables} (Tables~\ref{tab:summary_encoders} and~\ref{tab:summary_vlm}). The key finding is that no single method dominates all three regimes: BoF/Chamfer wins scene matching (VCDB AP\,=\,0.989), V-JEPA~2 temporal residual wins motion retrieval (HDD AP\,=\,0.956), and all models---including 31B-parameter VLMs---remain near chance on temporal order classification.
...
\textbf{Motion dynamics} (HDD, nuScenes). Per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56). On HDD, 89\% of the BoT-to-residual gap traces to the matching stage (cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}); on nuScenes the split inverts: encoder-sequence DTW (0.867) exceeds the residual (0.815, marginally overlapping CIs), so matching alone accounts for the full BoT$\to$best-method gap there. Direction replicates across datasets; magnitude does not. ViCLIP~\cite{viclip} and X-CLIP~\cite{xclip} do not escape the blind spot; explicit temporal training fares no better: TARA~\cite{tara} (HDD AP\,=\,0.547) and PL-Stitch~\cite{plstitch} under DTW (0.540, $s_\text{rev}\!=\!0.006$) trail DINOv3 Chamfer (0.559).

\textbf{Temporal manipulation} (reversal, scrambling). Implicit 3D models~\cite{3dcsl}, frame-sampling defenses~\cite{fojcik2025}, and VLMs all remain order-invariant~\cite{plstitch, aot}. Chiral training (TARA, $s_\text{rev}\!=\!0.955$) and a reasoning VLM (Gemini~3.1 Pro, $0.599$) improve over baselines but neither is reliable.
...
\State If AP degrades, run \emph{feature-vs-comparator decomposition}: attribute gap to encoder or matching stage.
\State Run \emph{sampling-density sweep}; isolate frame-rate / context-window effects.
\State Surface evaluation-protocol and behavioral failure modes via cluster-level CIs and generative-vs-embedding probe disagreement (class definitions in Appendix~\ref{sec:failure_taxonomy}).
\end{algorithmic}
\end{algorithm}
...
Temporal robustness & Attention-trajectory DTW & Detects reversal; not indexable \\
\bottomrule
\end{tabular}
\end{table}
...
No single method spans all three retrieval regimes. For copy detection, order-invariant aggregators (BoF, Chamfer) dominate; temporal comparators add noise. For motion retrieval, aggregation is the bottleneck: within a given encoder, switching the comparator from pooled-cosine to per-frame DTW explains 89\% of the HDD performance gap (point estimate; cluster-level CIs wider, Appendix~\ref{sec:bootstrap_cis}; direction replicates on nuScenes). For temporal robustness, no pooled readout---including 31B-parameter VLMs---distinguishes forward from reverse. These findings compose into a ten-class failure taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by an executable protocol (Algorithm~\ref{alg:decision}).
...
\begin{flushleft}
\small --- = not evaluated (we omit Encoder-Seq DTW on VCDB because BoF already saturates at AP\,=\,0.989; the matching stage is not the bottleneck in copy detection). VCDB AP depends on negative seed ($\pm$0.06; Appendix~\ref{sec:neg_sampling}). $^\ddagger$By construction: temporal derivatives negate under reversal, making DTW distance an odd function of frame ordering.
\end{flushleft}
\end{table}
