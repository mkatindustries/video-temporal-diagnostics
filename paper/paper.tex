...
that, within a given encoder, the comparator (not the encoder) governs
...
pooled readout of LLM hidden states recovers it (126 linear- and MLP-probe configurations across 3 VLMs and 7 pooling strategies). Generative
...
\textbf{Motion dynamics} (HDD, nuScenes). Per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56). The comparator dominates both motion benchmarks: it explains 89\% of the BoT-to-residual gap on HDD (cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}) and the full BoT$\to$best-method gap on nuScenes. Encoder-side magnitudes are dataset-dependent: on HDD the V-JEPA~2 residual $>$ encoder-seq DTW; on nuScenes encoder-seq DTW (0.867) $>$ residual (0.815, marginally overlapping CIs). Direction replicates; magnitude does not. ViCLIP~\cite{viclip} and X-CLIP~\cite{xclip} do not escape the blind spot; explicit temporal training fares no better: TARA~\cite{tara} (HDD AP\,=\,0.547) and PL-Stitch~\cite{plstitch} under DTW (0.540, $s_\text{rev}\!=\!0.006$) trail DINOv3 Chamfer (0.559).
