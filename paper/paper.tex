...
\begin{abstract}
Video retrieval embeddings collapse motion direction. Two videos of the same road in
opposite directions, or left versus right turns at the same intersection, return
near-identical embeddings---the numerical fingerprints used for similarity search---and
no downstream signal flags the failure. The cause is architectural: pooled aggregators
(bag-of-frames, BoF; bag-of-tokens, BoT) treat frames as an unordered set. We diagnose
this across seven benchmarks. A feature-vs-comparator decomposition shows that, within a
given encoder, the comparator (not the encoder) governs temporal sensitivity: on Honda
HDD, switching from pooled-cosine to per-frame dynamic time warping explains 89\% of
the gap between BoT and the V-JEPA~2 temporal residual (point estimate; cluster-level
CIs 7--32$\times$ wider than pair-level; direction replicates on nuScenes). Two
diagnostic instruments---a temporal scramble gradient and a reversal test---cleanly
separate order-invariant from sequence-aware methods; BoT yields cosine similarity
0.984 between forward and reversed clips, confirming the failure is by design. Probing
three open-weight VLMs localizes where temporal signal dies: per-frame vision features
retain order, mean-pooling erases it, and no pooled readout of LLM hidden states
recovers it (126 probe configurations). Generative probes on Claude~Opus~4.6 and
Gemini~3.1 Pro fare no better than 7B open-weight models. We consolidate these failures
into a 10-class taxonomy with an executable diagnostic protocol
(Algorithm~\ref{alg:decision}). No single approach spans both copy detection and motion
retrieval, exposing a sensitivity--invariance trade-off that current architectures do
not resolve. We release open-source code packaging the scramble gradient and reversal
tests.
\end{abstract}

\section{Introduction}

Video retrieval embeddings collapse motion direction: cars driving the same road in opposite directions retrieve as near-duplicates, and left and right turns at the same intersection return identical vectors. The cause is architecture, not training: modern systems represent videos via the \emph{bag-of-frames} (BoF) approach---mean-pooling (element-wise averaging) frame embeddings from pretrained encoders~\cite{faiss, semdedup}---which discards order by construction. BoF dominates Video Copy Detection (VCD; finding re-encoded, cropped, or camcorded copies) because visual semantics survive frame reordering, and order-invariance enables billion-scale nearest-neighbor search. The same property that makes BoF fast makes it blind to motion.
...
Beyond retrieval, temporal sensitivity is a prerequisite for world models~\cite{lecun_worldmodel}. A representation that cannot distinguish forward from reverse cannot predict future states from past observations. V-JEPA~2~\cite{vjepa2}---explicitly designed as a predictive world model---is the only encoder in our suite that preserves temporal order in its pooled representation. Our diagnostic framework thus measures a necessary condition for world model competence, not just retrieval quality.

We evaluate these cases with two primary encoders: DINOv3~\cite{dinov3} (image-SSL ViT-L; BoF and DTW attention trajectories) and V-JEPA~2~\cite{vjepa2} (Video Joint-Embedding Predictive Architecture v2; spatiotemporal prediction residuals). Three temporal-training controls probe whether specialized objectives help: ViCLIP~\cite{viclip} (video-native contrastive), TARA~\cite{tara} (a multimodal LLM fine-tuned with chiral---i.e., orientation-reversed---negatives), and PL-Stitch~\cite{plstitch} (Plackett-Luce temporal ranking). Three VLMs (Qwen3-VL~\cite{qwen3vl}, Gemma~4~\cite{gemma4}, LLaVA-Video~\cite{llavavideo}) probe whether LLM backbones preserve temporal signal. Benchmarks span seven domains: VCDB~\cite{vcdb} (near-duplicate video copy detection), Honda HDD~\cite{honda_hdd} (ego-vehicle driving maneuvers), nuScenes~\cite{nuscenes} (autonomous driving), Something-Something V2~\cite{ssv2} (crowd-sourced tabletop manipulation; non-driving cross-domain test), Nymeria~\cite{nymeria} (egocentric daily activity), MUVR~\cite{muvr} (multi-modal untrimmed video retrieval), and EPIC-Kitchens-100~\cite{epickitchens} (egocentric kitchen procedures).

\textbf{Contributions.} We contribute a diagnostic methodology for temporal sensitivity in video retrieval: (1)~a continuous \emph{temporal scramble gradient} separating order-invariant from sequence-aware methods (Figure~\ref{fig:scramble_gradient}); (2)~a \emph{feature-vs-comparator decomposition} with VLM-pipeline probes that localize where order signal dies, extending temporal-blindness diagnosis from generative QA to retrieval; (3)~a sampling-density analysis on Honda HDD; (4)~open-source tooling packaging the scramble gradient and reversal tests. Applied across seven benchmarks, the suite surfaces a sensitivity--invariance trade-off current architectures do not resolve (summarized as a failure-mode taxonomy, Appendix~\ref{sec:failure_taxonomy}).

\section{Methods}

\textbf{Why BoF is blind.}
Mean-pooling reversed frames gives the same vector as mean-pooling the original: the operator discards the indices. More generally, mean-pooling, max-pooling, and Chamfer matching are \emph{symmetric aggregators}: $g(f_1, \ldots, f_T) = g(f_{\pi(1)}, \ldots, f_{\pi(T)})$ for any permutation $\pi$~\cite{deepsets}---e.g., $\mathrm{mean}([f_1, f_2, f_3]) = \mathrm{mean}([f_3, f_1, f_2])$. BoF averages frame embeddings from an image encoder; BoT averages patch-level tokens from a spatiotemporal encoder. Both are symmetric aggregators, independent of encoder capacity or training objective.

\textbf{Feature-vs-comparator decomposition.}
We distinguish the encoder (\emph{features}) from the \emph{comparator}---the full similarity function (cosine over BoF, DTW over per-frame sequences, etc.); the aggregator is one stage inside the comparator. The decomposition fixes one and varies the other to attribute AP gaps. The diagnostics below distinguish architectural blindness (the aggregator) from learned blindness (the encoder).

\textbf{DINOv3 \& Attention Trajectories.} We extract 1024-d CLS embeddings per frame from DINOv3 ViT-L~\cite{dinov3}. \emph{BoF} mean-pools frame embeddings into a single vector, compared via cosine similarity. \emph{Chamfer similarity}~\cite{kordopatis2019visil} computes the pairwise frame similarity matrix and averages the symmetric maximum: $\frac{1}{2}\bigl(\mathrm{mean}_i \max_j S_{ij} + \mathrm{mean}_j \max_i S_{ij}\bigr)$, remaining order-invariant while retaining per-frame structure. For structural tracking, we compute the spatial center-of-mass of the CLS$\to$patch attention map (last layer, averaged across heads) per frame, yielding a 2D trajectory compared via DTW. Temporal derivatives of CLS embeddings capture motion, compared via DTW; by construction they negate under reversal, making the distance an odd function of frame order.

\textbf{V-JEPA~2 Temporal Residuals.} V-JEPA~2 ViT-L~\cite{vjepa2} ingests 64 frames as $32 \times 16 \times 16$ spatiotemporal patches ($D\!=\!1024$). The \emph{prediction residual} $R = \hat{P} - P$ (predicted minus actual masked patches) captures motion-specific error patterns. We spatially average each temporal position to obtain $\bar{R} \in \mathbb{R}^{T_\text{mask} \times D}$, compared via DTW with similarity $\exp(-d_\text{DTW})$. The \emph{bag-of-tokens} (BoT) baseline mean-pools all encoder patches into a single vector (cosine similarity).
...
\textbf{TARA.} TARA~\cite{tara} fine-tunes a Tarsier-7B multimodal LLM with chiral (orientation-reversed) negatives for temporal retrieval, encoding 16 frames into a single 4096-d vector via end-of-line prompting (cosine similarity).
...
\textbf{VLM Embedding Probes.} These are \emph{diagnostic procedures}, not new models: each probe reads a representation from an existing VLM pipeline and emits two metrics ($s_\text{rev}$ and balanced accuracy on forward/reverse classification). We probe three stages: (1)~\emph{vision pooled}, the mean-pooled vision tower output (BoF over frame$\times$patch tokens); (2)~\emph{vision sequence}, per-frame pooled vision tokens as a $(T, D)$ sequence compared via temporal derivative DTW; (3)~\emph{LLM hidden state}, the mean-pooled last-layer hidden state over all input tokens. We measure order sensitivity directly with $s_\text{rev}$: how similar is a clip's representation to its own reversed version? A value of 1.0 means the method cannot tell forward from backward; lower values mean it can. Formally, for representations $\mathbf{z}$ (single vector) or $\mathbf{Z}$ (sequence of vectors):
\begin{equation}
s_\text{rev} =
\begin{cases}
...
\label{eq:srev}
\end{equation}
where $\alpha$ follows the DTW $\alpha$ schedule defined below. Raw $s_\text{rev}$ values exceeding 1.0 (from f32 cosine precision) are clamped to 1.000 in all tables and figures; affected entries are marked $^\dagger$. \textbf{Cross-family caveat.} Pooled and sequence $s_\text{rev}$ use different similarity functions (cosine vs.\ $\exp(-\alpha d_\text{DTW})$) and are not directly comparable: rankings are valid \emph{within} each family, not across. Figure~\ref{fig:sensitivity_invariance} plots both on one axis as a presentational convenience.

\textbf{DTW Implementation.} Dynamic time warping aligns two sequences via a minimum-cost monotonic path that can stretch time but not reverse it, so it penalizes order violations that pooled cosine ignores. All comparisons use unconstrained warping with L2 cost matrices, path-length normalization ($d / (T_1 + T_2)$), and GPU-vectorized anti-diagonal wavefront processing~\cite{ddtw}. Similarity is $\exp(-\alpha \cdot d_\text{DTW})$ with $\alpha = 1$ for derivatives/residuals, $\alpha = 5$ for attention trajectories (min-max normalized to $[0,1]$). Because $\exp(-\alpha \cdot d)$ is monotone in $d$, rankings are $\alpha$-invariant; we verified this across three representative cells with $\alpha \in \{0.5, 1, 2, 5, 10\}$ (max AP spread $< 10^{-3}$).

\section{Experiments and Results}

\textbf{Evaluation protocols.} We report average precision (AP; area under the precision--recall curve) and, where applicable, area under the ROC curve (AUC) as retrieval metrics. All evaluations are pairwise. \emph{VCDB}: 528 videos; 5{,}585 positive pairs from annotations, negatives sampled 1:1 with fixed seed (sensitivity analysis across 1:1, 1:5, and all-pairs in Appendix~\ref{sec:neg_sampling}). \emph{HDD}: all $\binom{n}{2}$ pairs within each GPS-clustered intersection (DBSCAN density clustering, $\epsilon\!\approx\!30$\,m); positive = same maneuver; 50 mixed clusters, 1{,}687 maneuvers. \emph{nuScenes}: same protocol on ego\_pose; CAN-bus (Controller Area Network) turn labels; 264 segments, 950 pairs. \emph{EPIC s\textsubscript{rev}}: per-sequence self-comparison on 500 sequences; bootstrap 95\% CIs (1{,}000 resamples). \emph{Nymeria}: 100 egocentric sessions, 8{,}780 segments; positive = same activity same room; all $\binom{n}{2}$ pairs (394{,}210). \emph{MUVR News}: 9{,}958 videos, 74 topics, 27{,}453 positive pairs. \emph{SSv2}: 400 clips across chiral template pairs (50 per template); within-pair pairwise eval where positive = same template (same direction) and negative = chiral twin template (opposite direction).
...
DINOv3 methods fail (AP $\approx$ 0.50): dashcam attention tracks salient objects identical for both turns. V-JEPA~2 temporal residuals achieve AP\,=\,0.956; a pixel-difference motion proxy rules out a trivial intensity shortcut (Spearman $r = 0.051$; Appendix~\ref{sec:hdd_leakage}), and optical flow (RAFT) achieves only AP\,=\,0.478. Restricting to left-vs-right pairs yields AP\,=\,0.960 [0.958, 0.963]. Comparing encoder patches via DTW (encoder-sequence DTW) achieves AP\,=\,0.942, showing that the comparator (pooled-cosine $\to$ per-frame DTW) explains 89\% of the BoT-to-residual gap (0.825 $\to$ 0.956; point estimate; cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}). A pairwise \emph{symmetric-max} (Chamfer) aggregation~\cite{kordopatis2019visil} on the same V-JEPA~2 encoder-sequence patches further decomposes this: it reaches AP\,=\,0.914 without any dynamic programming, closing 76\% of the BoT$\to$Encoder-Seq-DTW gap. The remaining 24\% (0.914$\to$0.942) is attributable specifically to DTW's monotonic warping constraint. We restrict encoder-sequence DTW to motion tasks (HDD, nuScenes); on VCDB, BoF already hits AP\,=\,0.989.

\textbf{Does the decomposition yield a fast retriever?} We test the standard two-stage retriever over the full 1{,}687-segment corpus: V-JEPA~2 BoT top-$k$ pre-filter, encoder-sequence DTW rerank on survivors, non-survivors scored by a sentinel below the BoT floor (full sweep and RRF alternative in Appendix~\ref{sec:rerank_appendix}). The budget is harsher than a naive reading of the 89\% gap would predict. At $k\!=\!100$ the rerank collapses (AP\,=\,0.638) because pre-filter recall is only 0.35. At $k\!=\!1000$ ($\sim\!60\%$ of the corpus) AP reaches 0.934, within 0.008 of full DTW; the two-stage pipeline \emph{is} lossless at sufficient candidate budget. The pre-filter's similarity (appearance) is orthogonal to the task's (maneuver): same-intersection opposite-turn pairs crowd the top-$k$ and evict true positives. The decomposition is clean pairwise but does not decompose into a cheap indexable retriever via appearance-based filters; motion-aware first-stage filtering remains open.

\begin{table}[ht]
\centering
\caption{BoT$\to$DTW rerank on HDD. Non-survivors scored below the BoT floor (sentinel); full-sweep and RRF alternative in Appendix~\ref{sec:rerank_appendix}.}
\label{tab:rerank_hdd}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{AP} & \textbf{recall@$k$} & \textbf{DTW ops / query} \\
\midrule
V-JEPA~2 BoT only & 0.825 & --- & 0 \\
\; + DTW rerank, $k\!=\!100$ & 0.638 & 0.350 & 100 \\
\; + DTW rerank, $k\!=\!500$ & 0.862 & 0.776 & 500 \\
\; + DTW rerank, $k\!=\!1000$ & \textbf{0.934} & 0.957 & 1{,}000 \\
V-JEPA~2 Encoder-Seq DTW (full) & 0.942 & 1.000 & $\sim$1{,}687 \\
\bottomrule
\end{tabular}
\end{table}

TARA~\cite{tara}, a 7B MLLM trained with chiral negatives, hits AP\,=\,0.547 under cosine, indistinguishable from ViCLIP (0.542) despite ${\sim}23\times$ more parameters. PL-Stitch~\cite{plstitch} (ViT-Base, Plackett-Luce ranking) supports the same decomposition: BoF yields AP\,=\,0.478 and DTW on raw embeddings 0.540. Both sit near chance and below DINOv3 Chamfer (0.559), possibly reflecting a capacity (86M vs.\ 300M) and pretraining-scale gap rather than a comparator effect. Yet $s_\text{rev}\!=\!0.006$ under raw DTW confirms PL-Stitch \emph{does} encode frame order; the features are not discriminative enough for maneuver retrieval.

\subsubsection{Sampling Density: Context Window Sweep}
\label{sec:context_sweep}

We fix the backbone and pair set on HDD and sweep clip duration $d \in \{1.0, \ldots, 8.0\}$ seconds, always extracting 64 unique frames ($\text{fps}_\text{eff} = 64 / d$). This avoids the padding confound of the FPS downsample experiment (Appendix~\ref{sec:fps_invariance}). V-JEPA~2 temporal residual degrades more slowly than BoT as context widens; the residual-vs-BoT gap grows from $+0.038$ at 1\,s to $+0.224$ at 8\,s (Appendix~\ref{sec:context_sweep_table}, Figure~\ref{fig:context_sweep}).
...
\end{tabular}
\begin{flushleft}
\small $^*$DINOv3 300M params, CLIP (ViT-L) 304M, SigLIP (SO-400M) 400M. Order-invariance follows from the aggregation operator~\cite{deepsets}, not model capacity. $^\dagger$Raw values 1.001--1.002 from f32 precision; clamped to 1.000. We omit Qwen3-VL vision probes from this table because its ViT uses dynamic-resolution spatial grids with 3D temporal--spatial merging, which disrupts clean per-frame token grouping for the detailed $s_\text{rev}$ analysis; the mechanism behind its anomalously low pooled VCDB AP (0.491 vs.\ 0.915--0.926 for CLIP/SigLIP; Table~\ref{tab:summary_vlm}) warrants further investigation. Approximate pooled and sequence results appear in Table~\ref{tab:summary_vlm}. The LLM hidden-state probe (below) is unaffected.
\end{flushleft}
\end{table}

\textbf{Order signal is recoverable from per-frame features but erased by pooling.} Pooled VLM vision embeddings are order-invariant ($s_\text{rev} \approx 1.0$); retaining per-frame tokens and comparing via temporal derivative DTW achieves perfect forward/reverse separation (balanced accuracy\,=\,1.000). The LLM backbone does not recover this signal under any pooled readout ($s_\text{rev} \approx 1.0$ across all layers; Appendix~\ref{sec:layer_ablation}), nor do 126 probe configurations (logistic regression and MLP) on per-position hidden states (best: 0.560, within chance range; Appendix~\ref{sec:linear_probe}). Yet per-position hidden states \emph{do} retain order under full-sequence DTW (Appendix~\ref{sec:vision_token_probe}). Symmetric aggregation, not the backbone, is the bottleneck.

\subsection{Cross-Method Summary}

Full cross-method comparisons across all seven benchmarks are in Appendix~\ref{sec:cross_method_tables} (Tables~\ref{tab:summary_encoders} and~\ref{tab:summary_vlm}). The key finding is that no single method dominates all three regimes: BoF/Chamfer wins scene matching (VCDB AP\,=\,0.989), V-JEPA~2 temporal residual wins motion retrieval (HDD AP\,=\,0.956), and all models---including 31B-parameter VLMs---remain near chance on temporal order classification.
...
\textbf{Motion dynamics} (HDD, nuScenes, SSv2). Per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56). The comparator dominates motion benchmarks across domains: it explains 89\% of the BoT-to-residual gap on HDD (cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}), the full BoT$\to$best-method gap on nuScenes, and the full chance$\to$0.720 gap on SSv2 (DINOv3 attention trajectories vs.\ BoF\,$=$\,0.500). \emph{Which} comparator wins is domain-dependent: V-JEPA~2 residuals lead on HDD (0.956), encoder-seq DTW leads on nuScenes (0.867), and attention trajectories lead on SSv2 (0.720) where the residual shows only modest transfer (0.556). The split is interpretable: residuals capture ego-motion dynamics through shared road geometry, while attention trajectories track object-motion direction across varied tabletop scenes; SSv2 temporal-derivative DTW at chance (0.503) confirms frame-differencing detects reversal but not cross-instance direction. Direction replicates across datasets; the optimal comparator does not. ViCLIP~\cite{viclip} and X-CLIP~\cite{xclip} do not escape the blind spot; explicit temporal training fares no better: TARA~\cite{tara} (HDD AP\,=\,0.547) and PL-Stitch~\cite{plstitch} under DTW (0.540, $s_\text{rev}\!=\!0.006$) trail DINOv3 Chamfer (0.559).

\textbf{Temporal manipulation} (reversal, scrambling). Implicit 3D models~\cite{3dcsl}, frame-sampling defenses~\cite{fojcik2025}, and VLMs all remain order-invariant~\cite{plstitch, aot}. Chiral training (TARA, $s_\text{rev}\!=\!0.955$) and a reasoning VLM (Gemini~3.1 Pro, $0.599$, the only generative probe credibly above chance) improve over baselines but neither is deployment-reliable.

\textbf{Architecture implications.} Temporal blindness is a choice of aggregator, not a deficiency of the encoder. Indexable order-sensitive retrieval at scale will require learned permutation-sensitive pooling, trajectory tokenization compatible with inverted-file indexes, or two-stage rerankers whose first stage is \emph{motion-aware}: naive appearance pre-filters lose recall to intersection-identity collisions (Section~\ref{sec:motion}).

\textbf{World model implications.} V-JEPA~2's superior temporal sensitivity is consistent with its design as a predictive world model~\cite{lecun_worldmodel,vjepa2}: the self-supervised prediction objective implicitly enforces temporal ordering, because predicting the future requires knowing which direction time flows. This suggests the path to temporally sensitive retrieval runs through world model training objectives rather than retrieval-specific losses. Unified understanding-generation models (e.g., TUNA~\cite{tuna}) cannot afford temporal blindness because their generation objective requires temporal coherence---further evidence that the field is converging on representations that resolve this blindness. The sensitivity--invariance trade-off (Figure~\ref{fig:sensitivity_invariance}) thus doubles as a diagnostic for world model readiness.

\begin{algorithm}[h]
\caption{Temporal Sensitivity Diagnostic Protocol}
\label{alg:decision}
\begin{algorithmic}[1]
\State Run \emph{scramble gradient}. Flat AP $\Rightarrow$ architectural order-invariance; \textbf{stop}.
\State If AP degrades, run \emph{feature-vs-comparator decomposition}: attribute gap to encoder or comparator.
\State Run \emph{sampling-density sweep}; isolate frame-rate / context-window effects.
\State Cluster-level bootstrap CIs flag evaluation-protocol artifacts; generative-VLM answers vs.\ embedding $s_\text{rev}$ separates behavioral from architectural blindness (Appendix~\ref{sec:failure_taxonomy}).
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[ht]
    ...
    \caption{Sensitivity--invariance trade-off: $1 - s_\text{rev}$ (EPIC) vs.\ VCDB AP. No method occupies the upper-right quadrant. Pooled and sequence $s_\text{rev}$ use different similarity functions (Eq.~\eqref{eq:srev}); the quadrant claim depends on within-family rankings only.}
    \label{fig:sensitivity_invariance}
    \vspace{-0.5em}
\end{figure}

\textbf{Limitations.} DTW is not indexable ($O(T^2)$); probes train on 400 samples in $p \gg n$ ($D \approx 4{,}000$), bounding recoverability rather than proving absence; ViCLIP's 8-frame input confounds comparison with 64-frame V-JEPA~2 (${\sim}0.20$ AP drop at equivalent density, Section~\ref{sec:context_sweep}).
...
\begin{tabular}{lll}
\toprule
\textbf{Regime} & \textbf{Recommended approach} & \textbf{Trade-off} \\
\midrule
Copy detection & BoF / Chamfer & Order-invariance is a feature \\
Motion retrieval & BoT$\to$DTW rerank ($k\!\approx\!1000$) & 99\% of full-DTW AP; recall binds at small $k$ \\
Reversal-attack defense & Attention-trajectory DTW & Detects reversal; not indexable \\
\bottomrule
\end{tabular}
\end{minipage}
...
No single method spans all three retrieval regimes. For copy detection, order-invariant aggregators (BoF, Chamfer) dominate; temporal comparators add noise. For motion retrieval, the comparator is the bottleneck: within a given encoder, switching from pooled-cosine to per-frame DTW explains 89\% of the HDD performance gap (point estimate; cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}); direction replicates across HDD, nuScenes, and SSv2, but the comparator that recovers signal is domain-dependent (V-JEPA~2 residual on ego-motion driving; attention-trajectory DTW on static-camera manipulation). For reversal detection, no pooled readout---including 31B-parameter VLMs---distinguishes forward from reverse. These findings compose into a ten-class failure taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by an executable protocol (Algorithm~\ref{alg:decision}).

A two-stage BoT$\to$DTW reranker confirms the decomposition end-to-end but exposes an asymmetry: appearance-based pre-filters recover only 35\% of motion-retrieval positives at $k\!=\!100$, so the pairwise gap does not compose into a cheap retriever. At $k\!\approx\!1000$ (${\sim}60\%$ of the corpus) AP recovers to within 0.008 of full DTW (Table~\ref{tab:rerank_hdd}). The open problem is motion-aware indexing: designing first-stage filters whose notion of similarity aligns with dynamics, not appearance. V-JEPA~2's temporal sensitivity is consistent with, but does not isolate, the benefits of predictive training objectives~\cite{lecun_worldmodel,vjepa2}; testing this hypothesis across architectures is future work.%
\ifdefined\neuripssubmission
\else
...
\bibitem{lecun_worldmodel} Y.~LeCun. A Path Towards Autonomous Machine Intelligence. \emph{openreview.net preprint}, version~0.9.2, 2022.
...
\section{HDD Maneuver Discrimination Figure}
\label{sec:hdd_maneuver_figure}

Figure~\ref{fig:hdd_maneuver} visualizes the V-JEPA~2 rows of Table~\ref{tab:hdd_maneuver}: AP climbs sharply from BoT (0.825) through Chamfer (0.914) and Encoder-Seq DTW (0.942) to the temporal residual (0.956), while DINOv3 methods (BoF, Chamfer, temporal derivative, attention trajectory) all sit near chance ($\leq 0.56$). The ordering is the visual counterpart of the feature-vs-comparator decomposition reported in Section~\ref{sec:motion}.
...
Table~\ref{tab:context_sweep} and Figure~\ref{fig:context_sweep} report the Section~\ref{sec:context_sweep} sampling-density experiment: we vary clip duration $d \in \{1.0, 2.0, 3.0, 4.0, 6.5, 8.0\}$ seconds while extracting a fixed 64 unique frames per clip, sweeping the effective frame rate from 11.1 to 3.2~fps.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{../figures/hdd_context_sweep.png}
    \caption{V-JEPA~2 context sweep on Honda HDD (1{,}687 maneuvers, 128 sessions). Temporal residual degrades more slowly than BoT; the gap grows from $+0.038$ at 1\,s to $+0.224$ at 8\,s.}
...
\toprule
\textbf{Method} & \textbf{VCDB (AP$\uparrow$)} & \textbf{HDD (AP$\uparrow$)} & \textbf{nuScenes (AP$\uparrow$)} & \textbf{SSv2 (AP$\uparrow$)} & \textbf{EPIC $s_\text{rev}$$\downarrow$} & \textbf{EPIC Bal.\ Acc.$\uparrow$} \\
\midrule
DINOv3 BoF & 0.979 & 0.530 & 0.613 & 0.500 & 1.000 & N/A \\
DINOv3 Chamfer & \textbf{0.989} & 0.559 & 0.612 & 0.500 & 0.9999 & N/A \\
DINOv3 Temporal Deriv. & 0.775 & 0.498 & 0.623 & 0.503 & 0.500 & \textbf{1.000}$^\ddagger$ \\
DINOv3 Attn.\ Trajectory & 0.700 & 0.507 & 0.584 & \textbf{0.720} & \textbf{0.192} & \textbf{1.000}$^\ddagger$ \\
V-JEPA 2 BoT & 0.838 & 0.825 & 0.791 & 0.555 & 0.984 & 0.500 \\
V-JEPA 2 Encoder-Seq DTW & --- & 0.942 & \textbf{0.867} & --- & --- & --- \\
V-JEPA 2 Temporal Res. & 0.698 & \textbf{0.956} & 0.815 & 0.556 & 0.699 & 0.778 \\
...
\begin{flushleft}
\small --- = not evaluated (we omit Encoder-Seq DTW on VCDB because BoF already saturates at AP\,=\,0.989; the comparator is not the bottleneck in copy detection). VCDB AP depends on negative seed ($\pm$0.06; Appendix~\ref{sec:neg_sampling}). $^\ddagger$By construction: temporal derivatives negate under reversal, making DTW distance an odd function of frame ordering.
\end{flushleft}
\end{table}
...
\section{BoT$\to$DTW Rerank: Full Sweep and RRF Alternative}
\label{sec:rerank_appendix}

Table~\ref{tab:rerank_full} reports the full $k$-sweep for the two-stage retriever test of Section~\ref{sec:motion}, including a reciprocal-rank-fusion (RRF) composition as an alternative to the sentinel rule used in the main body. RRF computes, for each pair, $1/(60 + \mathrm{rank}_\text{BoT}) + \mathrm{[pair in top-}k\mathrm{]}/(60 + \mathrm{rank}_\text{DTW})$; it is robust to the BoT/DTW score-scale mismatch but more conservative than the sentinel rule because the BoT rank term still influences the composite when DTW is confident.

\begin{table}[h]
\centering
\caption{BoT$\to$DTW rerank on Honda HDD (43{,}673 in-cluster pairs; 1{,}687 segments across 50 mixed-direction clusters). \textbf{Survivor AP}: non-survivors assigned a sentinel below the BoT floor so they rank last. \textbf{RRF AP}: rank-fusion composition (see text). \textbf{recall@$k$}: fraction of positive pairs where at least one end survives the top-$k$ BoT filter. 95\% bootstrap CIs in brackets (1{,}000 resamples). Full DTW ceiling = 0.9416; BoT-only floor = 0.8246.}
\label{tab:rerank_full}
\begin{tabular}{rcccc}
\toprule
$k$ & recall@$k$ & Survivor AP & RRF AP & positives reranked \\
\midrule
10 & 0.094 & 0.499 [0.496, 0.502] & 0.825 [0.822, 0.828] & 4{,}101 / 43{,}673 \\
25 & 0.163 & 0.536 [0.533, 0.540] & 0.826 [0.823, 0.829] & 7{,}100 \\
50 & 0.238 & 0.578 [0.574, 0.581] & 0.829 [0.826, 0.832] & 10{,}382 \\
100 & 0.350 & 0.638 [0.635, 0.642] & 0.835 [0.832, 0.838] & 15{,}264 \\
250 & 0.568 & 0.755 [0.752, 0.758] & 0.855 [0.852, 0.858] & 24{,}802 \\
500 & 0.776 & 0.862 [0.860, 0.865] & 0.883 [0.880, 0.885] & 33{,}898 \\
1000 & 0.957 & \textbf{0.934 [0.933, 0.936]} & 0.910 [0.907, 0.912] & 41{,}783 \\
\bottomrule
\end{tabular}
\end{table}

Both curves are monotonically non-decreasing in $k$ until the approach to the ceiling. Survivor-AP is the cleaner measurement because it is the \emph{end-to-end retriever} AP: pairs not surviving the pre-filter are scored below every survivor, which is faithful to the deployment semantics of a two-stage pipeline. RRF never reaches the full-DTW ceiling by construction (the BoT rank component pulls the composite down when DTW is already confident), but it never falls below BoT-only either, making it a safer drop-in when reranker calibration is uncertain. The reproducibility of these numbers depends on the feature cache used by Section~\ref{sec:motion} (see the \texttt{eval\_hdd\_bof\_dtw\_rerank.py} script in supplementary material).

\section{nuScenes Cross-Dataset Visualization}
\label{sec:nuscenes_figure}

Figure~\ref{fig:nuscenes_maneuver} visualizes the nuScenes maneuver-discrimination results reported in Section~\ref{sec:nuscenes}. Encoder-sequence DTW (AP\,=\,0.867) leads, followed by the temporal residual (AP\,=\,0.815) and BoT (AP\,=\,0.791), with DINOv3 methods below 0.625. The ordering makes concrete the Section~\ref{sec:discussion} observation that on nuScenes the matching-stage contribution saturates the BoT$\to$best-method gap by itself, so the residual transform adds nothing statistically distinguishable here.
...
Table~\ref{tab:scramble_data} gives the numerical backing for the main-body scramble gradient (Figure~\ref{fig:scramble_gradient}): AP on VCDB's 5{,}585 positive pairs across chunk counts $K \in \{1, 2, 3, 4, 6, 8, 12, 16\}$, where $K\!=\!1$ is the unscrambled baseline and higher $K$ means finer-grained temporal shuffling.
