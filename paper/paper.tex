...
the encoder, governs temporal sensitivity: on Honda HDD, the comparator
(cosine$\to$DTW) explains 89\% of the performance gap; on nuScenes, the
...
($s_\text{rev} \approx 1.0$). Neither linear nor nonlinear probes recover
...
On copy detection, BoF and Chamfer dominate: DINOv3 Chamfer reaches AP\,=\,0.989 (528 videos, 5{,}585 positive pairs); temporal methods underperform on frame-to-frame intra-class noise. Nymeria and MUVR replicate the ranking (Appendix~\ref{sec:scene_matching}).
...
To test if the ranking transfers, we evaluate on nuScenes~\cite{nuscenes}. Turn labels come from CAN bus; intersections are clustered via DBSCAN ($\epsilon\!=\!30$m), yielding 264 segments across 50 mixed-direction clusters (950 pairs).

The ranking is directionally consistent (Table~\ref{tab:summary_encoders}): V-JEPA~2 encoder-sequence DTW hits AP\,=\,0.867, surpassing the temporal residual (0.815) and BoT (0.791); DINOv3 methods stay $\leq$0.623. Encoder-sequence DTW beats the residual here, confirming the comparator as the primary driver. CIs overlap marginally ([0.842,\,0.890] vs.\ [0.785,\,0.844]; Appendix~\ref{sec:bootstrap_cis}). The residual helps on HDD but not nuScenes, likely because nuScenes' native 2\,Hz yields sparser temporal gradients where prediction errors add noise; however, nuScenes at 2\,Hz still beats HDD subsampled to 2\,FPS with repeat-padding (AP\,=\,0.636), suggesting native keyframes preserve more temporal signal (Figure~\ref{fig:nuscenes_maneuver}, Appendix~\ref{sec:nuscenes_figure}).
...
VLMs \emph{output} answers carrying no temporal information. We probe whether temporal signal persists in internal representations before being discarded.
...
No single method dominates. \textbf{(i)~Scene matching} (VCDB, Nymeria, MUVR): BoF wins; temporal methods add noise (Appendix~\ref{sec:scene_matching}). \textbf{(ii)~Motion dynamics} (HDD, nuScenes): per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56); the residual advantage grows under sparser sampling (Section~\ref{sec:context_sweep}). On HDD, 89\% of the BoT-to-residual gap traces to the comparator (cosine$\to$DTW); on nuScenes, encoder-sequence DTW alone surpasses the residual transform (AP\,=\,0.867 vs.\ 0.815), confirming the comparator as the primary driver. Video-native ViCLIP~\cite{viclip} ($s_\text{rev}\!=\!0.996$, HDD AP\,=\,0.542) and X-CLIP~\cite{xclip} ($s_\text{rev}\!=\!0.980$) do not escape the blind spot (ViCLIP's 8-frame input partly confounds this, though it reflects the model's designed operating regime; see Limitations). Encoders with explicit temporal training fare no better under symmetric comparators: TARA~\cite{tara} (chiral negatives, HDD AP\,=\,0.547) and PL-Stitch~\cite{plstitch} BoF (AP\,=\,0.478) fail under cosine; PL-Stitch under DTW (0.540) still trails DINOv3 Chamfer (0.559), though $s_\text{rev}\!=\!0.006$ confirms it encodes order---likely a capacity (86M vs.\ 300M) and pretraining-scale gap, not a comparator failure. \textbf{(iii)~Temporal manipulation} (reversal, scrambling): implicit 3D models~\cite{3dcsl}, frame-sampling defenses~\cite{fojcik2025}, and Video-LLMs all remain order-invariant~\cite{plstitch, aot}. TARA's chiral training lowers $s_\text{rev}$ to 0.955 (vs.\ 0.996 for ViCLIP)---improved but insufficient; a reasoning VLM (Gemini~3.1 Pro) reaches $0.599$ vs.\ ${\leq}0.545$ for non-reasoning models. Per-frame features separate forward/reverse under DTW (by construction); pooling erases it, and LLM hidden states remain order-invariant across all layers.
...
\textbf{Limitations.} All evaluations use pairwise similarity; DTW is not indexable ($O(T^2)$); probes train on 400 samples ($p \gg n$); ViCLIP's 8-frame input confounds its comparison with 64-frame V-JEPA~2 (Section~\ref{sec:context_sweep} provides a partial control); we maintain this 8-frame limit as it represents ViCLIP's intended operating regime. Temporal manipulation is one axis of the copy-detection attack surface; spatial occlusion attacks (emoji/logo overlays, watermarks, color-channel perturbations) remain a separate and, in production workflows, often more prevalent threat---orthogonal to the temporal blindness we diagnose here.

\textbf{Practitioner recommendations.} (1)~Test your pipeline with the scramble gradient at $K \in \{1, 4, 16\}$ using \textsc{TemporalDiag} (\texttt{temporal-diag report}): if AP is flat across $K$, the system is temporally blind. (2)~For copy detection, match the method to the action cost: hard actions requiring precision~$\geq$~99\% (takedowns, account bans) favor BoF with a conservative threshold, where adding temporal methods hurts AP; soft actions where false positives are cheap (ranking demotions, review queues) can accept temporal-sequence comparators that flag reversal and structural manipulations. (3)~For motion-sensitive retrieval, a two-stage pipeline (BoF pre-filter $\to$ DTW rerank on top-$k$) is practical at ${\sim}100$\,ms overhead per query (Appendix~\ref{sec:computational_costs}).

\section{Conclusion}

Across six benchmarks, no single method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}). The temporal scramble gradient cleanly separates method classes; the feature-vs-comparator decomposition places 89\% of the HDD gap in the matching function, and on nuScenes the comparator alone (encoder-sequence DTW) surpasses the residual transform (AP\,=\,0.867 vs.\ 0.815). Encoders with temporal training---TARA~\cite{tara} (AP\,=\,0.547) and PL-Stitch~\cite{plstitch} (AP\,=\,0.540 under DTW)---also fail on HDD, confirming that the comparator, not the training objective, governs retrieval discrimination. VLM embedding probes show pooling erases the order information present in per-frame encoders (126 configurations at chance). Efficient order-sensitive comparators remain the key open problem.\footnote{%
\ifdefined\neuripssubmission
