...
BoF discards temporal order entirely. A growing literature documents this ``temporal blindness'' in Video-LLMs~\cite{tomato,temporalbench,tempcompass,infact,videozerobench,vbenchcomp}: generative probes show near-chance temporal QA; frame shuffling leaves benchmark scores intact. These diagnoses target generative QA, where failures surface in text. In retrieval, failures are silent---a reversed video returns the same embedding, a left turn matches a right turn---and no downstream signal flags them. We fill this gap with three test cases:
...
\textbf{Contributions.} (1)~A \emph{temporal scramble gradient} that stress-tests temporal sensitivity in retrieval embeddings and separates method classes on VCDB (Figure~\ref{fig:scramble_gradient}). (2)~A \emph{feature-vs-comparator decomposition} and VLM-pipeline embedding probes that localize where order signal dies (89\% in the HDD comparator, not the encoder), extending temporal-blindness diagnosis from generative QA to retrieval. (3)~A sampling-density analysis on Honda HDD that tracks how the residual advantage scales with temporal dilution. (4)~\textsc{TemporalDiag}, an open-source toolkit (CLI: \texttt{temporal-diag}) packaging the scramble gradient and reversal test.
...
On copy detection, BoF and Chamfer dominate: DINOv3 Chamfer reaches AP\,=\,0.989 (528 videos, 5,585 positive pairs); temporal methods underperform due to intra-class noise in frame-to-frame dynamics. The ranking replicates on Nymeria and MUVR (Appendix~\ref{sec:scene_matching}).
