...
% Target track: NeurIPS 2026 Evaluations & Datasets (E&D), single-blind.
% ----- ----------------------------------------------------------------
\ifdefined\neuripssubmission
  \usepackage[eandd,nonanonymous,nonatbib]{neurips_2026} % E&D track, single-blind
\else
...
\begin{abstract}
Video retrieval embeddings collapse motion direction by design: two videos of the same
road in opposite directions, or left versus right turns at the same intersection,
return near-identical embeddings, and no retrieval-pipeline metric flags the failure.
We introduce a diagnostic framework for measuring temporal sensitivity in video
retrieval, comprising three executable instruments: a temporal scramble gradient
($K\!\in\!\{1,\ldots,16\}$), a forward/reverse invariance test producing a reversal
similarity $s_\text{rev}$, and a feature-vs-comparator decomposition that attributes AP
gaps to the encoder or the comparator. Applied across seven benchmarks spanning copy
detection, ego-motion driving, object manipulation, and egocentric activity, plus three
open-weight VLMs, the framework localizes where temporal information is preserved or
destroyed within modern pipelines. Symmetric pooling erases order architecturally: bag-of-tokens (BoT) mean-pooling yields
reversal similarity $s_\text{rev}\!\approx\!1$ between forward and reversed clips by
construction. The decomposition shows that, within a given encoder, the comparator---not
the encoder---governs temporal sensitivity on motion retrieval: switching from pooled-cosine
to per-frame dynamic time warping explains 89\% of the BoT-to-residual gap on Honda HDD;
the direction replicates on nuScenes and Something-Something V2, but \emph{which}
comparator recovers temporal signal is domain-dependent. VLM probing further localizes the
loss: per-frame vision features retain order, pooling erases it, and no pooled
readout---including 31B-parameter LLM hidden states and generative probes on two reasoning
VLMs---recovers it. We consolidate these findings into a 10-mode failure taxonomy and an
executable diagnostic protocol (Algorithm~\ref{alg:decision}), and release an open-source
toolkit packaging the three instruments. The empirical contribution is a
sensitivity--invariance trade-off current architectures do not resolve.
\end{abstract}
...
We evaluate these cases with two primary encoders: DINOv3~\cite{dinov3} (image-SSL ViT-L; BoF and DTW attention trajectories) and V-JEPA~2~\cite{vjepa2} (Video Joint-Embedding Predictive Architecture v2; spatiotemporal prediction residuals). Three temporal-training controls probe whether specialized objectives help: ViCLIP~\cite{viclip} (video-native contrastive), TARA~\cite{tara} (a multimodal LLM fine-tuned with chiral---i.e., mirror-direction---negatives), and PL-Stitch~\cite{plstitch} (Plackett-Luce temporal ranking). Three VLMs (Qwen3-VL~\cite{qwen3vl}, Gemma~4~\cite{gemma4}, LLaVA-Video~\cite{llavavideo}) probe whether LLM backbones preserve temporal signal. Benchmarks span seven domains: VCDB~\cite{vcdb} (near-duplicate video copy detection), Honda HDD~\cite{honda_hdd} (ego-vehicle driving maneuvers), nuScenes~\cite{nuscenes} (autonomous driving), Something-Something V2~\cite{ssv2} (crowd-sourced tabletop manipulation; non-driving cross-domain test), Nymeria~\cite{nymeria} (egocentric daily activity), MUVR~\cite{muvr} (multi-modal untrimmed video retrieval), and EPIC-Kitchens-100~\cite{epickitchens} (egocentric kitchen procedures).

\textbf{Contributions.} This paper introduces a diagnostic framework for temporal sensitivity in video retrieval, comprising three executable instruments: (1)~a \emph{temporal scramble gradient} separating order-invariant from sequence-aware methods (Figure~\ref{fig:scramble_gradient}); (2)~a \emph{forward--reverse invariance test} producing a reversal similarity $s_\text{rev}$ comparable across methods; and (3)~a \emph{feature-vs-comparator decomposition} that attributes AP gaps to the encoder or to the comparator. Applied across seven benchmarks and three open-weight VLMs, the framework localizes where temporal information is preserved or destroyed and consolidates the findings into a 10-mode failure taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by an executable protocol (Algorithm~\ref{alg:decision}). We release an open-source toolkit packaging the three instruments. The empirical contribution is a sensitivity--invariance trade-off current architectures do not resolve: BoF wins copy detection by exactly the property that disqualifies it for motion retrieval, and the comparator that recovers temporal signal on motion is domain-dependent.

\section{Methods}
\label{sec:methods}

\textbf{Why BoF is blind.}
Mean-pooling is permutation-invariant: reversed frames yield the same vector. More generally, mean-pooling, max-pooling, and Chamfer matching are \emph{symmetric aggregators}~\cite{deepsets}: $g(f_1, \ldots, f_T) = g(f_{\pi(1)}, \ldots, f_{\pi(T)})$ for any permutation $\pi$. BoF averages frame embeddings from an image encoder; BoT averages patch-level tokens from a spatiotemporal encoder. Both are symmetric, independent of encoder capacity or training objective---the formal source of motion-direction blindness.
...
\emph{Pooled} $s_\text{rev}$ (cosine) and \emph{sequence} $s_\text{rev}$ ($\exp(-\alpha d_\text{DTW})$) use different similarity functions and are \textbf{not directly comparable}. Rankings are valid within each family, not across families. Figure~\ref{fig:sensitivity_invariance} plots both on a single axis as a presentational convenience; the ``no method in the upper-right quadrant'' claim depends only on within-family orderings.
\end{remark}
...
\begin{figure}[t]
    ...
    \caption{HDD AP across methods. V-JEPA~2 progression: BoT $0.825 \to$ Chamfer $0.914 \to$ Encoder-Seq DTW $0.942 \to$ Temporal Residual $0.956$ (89\% comparator-dominance recovered before any learned residual).}
    \label{fig:hdd_maneuver}
    \vspace{-1.2em}
\end{figure}

DINOv3 methods fail (AP $\approx$ 0.50): dashcam attention tracks salient objects identical for both turns. V-JEPA~2 temporal residuals achieve AP\,=\,0.956; a pixel-difference motion proxy rules out a trivial intensity shortcut (Spearman $r = 0.051$; Appendix~\ref{sec:hdd_leakage}), and optical flow (RAFT) achieves only AP\,=\,0.478. Restricting to left-vs-right pairs yields AP\,=\,0.960 [0.958, 0.963].

Encoder-sequence DTW---per-frame patch tokens aligned via DTW instead of mean-pooled---reaches AP\,=\,0.942, so switching pooled-cosine to per-frame DTW alone explains 89\% of the BoT-to-residual gap (0.825$\to$0.956; cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}; the comparator switch dominates even at the cluster-CI low end of the residual, $\geq$0.890). A symmetric-max (Chamfer) aggregation~\cite{kordopatis2019visil} on the same encoder-sequence patches reaches AP\,=\,0.914 without dynamic programming, closing 76\% of the BoT$\to$encoder-sequence DTW gap; the remaining 24\% (0.914$\to$0.942) comes from DTW's monotonic warping constraint. We restrict encoder-sequence DTW to motion tasks (HDD, nuScenes); on VCDB, BoF already hits AP\,=\,0.989.

\textbf{Diagnostic-protocol stress test: does the decomposition compose into a fast retriever?} The decomposition predicts that an indexable BoT pre-filter feeding a DTW reranker should approximate full encoder-sequence DTW at lower compute. We run this on the full 1{,}687-segment corpus (V-JEPA~2 BoT top-$k$ pre-filter, encoder-sequence DTW rerank on survivors, non-survivors scored by a sentinel below the BoT floor; full sweep and RRF alternative in Appendix~\ref{sec:rerank_appendix}). The protocol exposes a budget asymmetry that the pairwise 89\% number does not. At $k\!=\!100$ the rerank collapses (AP\,=\,0.638) because pre-filter recall is only 0.35. At $k\!=\!1000$ ($\sim\!60\%$ of the corpus) AP reaches 0.934, within 0.008 of full DTW; the two-stage pipeline \emph{is} lossless at sufficient candidate budget. The pre-filter's similarity (appearance) is orthogonal to the task's (maneuver): same-intersection opposite-turn pairs crowd the top-$k$ and evict true positives. \emph{This is a negative result the framework surfaces by construction}: the pairwise decomposition is clean, but the gap does not compose into a cheap indexable retriever via appearance-based filters. Motion-aware first-stage filtering is the concrete open problem the diagnostic exposes.
...
\begin{table}[ht]
\centering
\caption{VLM forward/reverse classification on EPIC-Kitchens (500 sequences for open-weight models, 200 for API models). All at or near chance. Metric and $\pm$ convention defined in \S\ref{sec:methods} (VLM Generative Probes); $\pm$ values are prompt-sensitivity SD, not statistical uncertainty.}
...
\textbf{Motion dynamics} (HDD, nuScenes, SSv2). Per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56). The comparator dominates across domains, but \emph{which} comparator wins varies: V-JEPA~2 residuals capture ego-motion dynamics on HDD and nuScenes; attention trajectories track object-motion direction on SSv2 tabletop scenes. Direction replicates; the optimal comparator does not. Temporal-training controls (TARA~\cite{tara}, PL-Stitch~\cite{plstitch}, ViCLIP~\cite{viclip}, X-CLIP~\cite{xclip}) do not escape the blind spot either.
...
\textbf{Architecture implications.} Temporal blindness is a choice of aggregator; indexable order-sensitive retrieval will require permutation-sensitive pooling, trajectory tokenization, or motion-aware rerankers (Section~\ref{sec:motion}).

\textbf{World model implications.} V-JEPA~2's temporal sensitivity is consistent with its design as a predictive world model~\cite{lecun_worldmodel,vjepa2} (forecasting requires temporal ordering); we do not isolate this as the cause, but the sensitivity--invariance trade-off (Figure~\ref{fig:sensitivity_invariance}) is a candidate diagnostic for any representation whose downstream use requires temporal order.
...
\begin{figure}[ht]
    \centering
    \vspace{-1.2em}
    \includegraphics[width=0.50\textwidth]{../figures/sensitivity_invariance_tradeoff.png}
    \caption{Sensitivity--invariance trade-off: $1 - s_\text{rev}$ (EPIC) vs.\ VCDB AP. No method occupies the upper-right quadrant; the claim is within-family (Eq.~\eqref{eq:srev}, Remark~1).}
    \label{fig:sensitivity_invariance}
    \vspace{-1.5em}
\end{figure}

\textbf{Limitations.} DTW is not indexable ($O(T^2)$); probes train on 400 samples in $p \gg n$ ($D \approx 4{,}000$), bounding recoverability rather than proving absence; ViCLIP's 8-frame input confounds comparison with 64-frame V-JEPA~2 (${\sim}0.20$ AP drop at equivalent density, Section~\ref{sec:context_sweep}); motion-retrieval evidence rests on three benchmarks (HDD, nuScenes, SSv2) with \emph{directional} (not absolute) cross-domain replication.


\section{Conclusion}

We introduced a diagnostic framework (three instruments, a 10-mode failure taxonomy, Algorithm~\ref{alg:decision}) that localizes where temporal information is preserved or destroyed in video retrieval. The central finding (Figure~\ref{fig:sensitivity_invariance}) is an unresolved sensitivity--invariance trade-off: BoF/Chamfer's order-invariance powers VCDB copy detection (AP\,=\,0.989) but is the architectural source of motion-direction blindness, and no current architecture occupies both regimes. The comparator dominates motion benchmarks with the optimal choice domain-dependent, and reversal detection remains out of reach for all pooled readouts. The protocol surfaces a negative result naming the next step: an indexable BoT$\to$DTW retriever collapses at $k\!=\!100$ (recall 0.35), recovering only at $k\!\approx\!1000$. Motion-aware first-stage filtering remains the open problem.\footnote{Toolkit code: \url{https://github.com/mkatindustries/video-temporal-diagnostics}.}
