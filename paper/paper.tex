...
\textbf{Motion dynamics} (HDD, nuScenes). Per-frame methods and VLM vision towers fail (AP\,$\leq$\,0.56). On HDD, 89\% of the BoT-to-residual gap traces to the matching stage (cluster-level CIs 7--32$\times$ wider than pair-level, Appendix~\ref{sec:bootstrap_cis}); on nuScenes the split inverts---encoder-sequence DTW (0.867) exceeds the residual (0.815, marginally overlapping CIs), so matching alone accounts for the full BoT$\to$best-method gap there. The HDD 89\% is a single-dataset point estimate; what replicates across datasets is the direction (matching dominates), not the precise share. ViCLIP~\cite{viclip} and X-CLIP~\cite{xclip} do not escape the blind spot; explicit temporal training fares no better: TARA~\cite{tara} (HDD AP\,=\,0.547) and PL-Stitch~\cite{plstitch} under DTW (0.540, $s_\text{rev}\!=\!0.006$) trail DINOv3 Chamfer (0.559). V-JEPA~2's spatiotemporal masked-prediction supervises motion; image-SSL and temporal-ranking objectives alone are insufficient without adequate capacity and pretraining scale.
...
No method spans both copy detection and motion retrieval (Figure~\ref{fig:sensitivity_invariance}): within a given encoder, matching---not encoding---explains 89\% of the HDD gap (single-dataset point estimate; nuScenes yields a directionally consistent but smaller split with overlapping CIs), and pooling erases per-frame order across every VLM layer we probed. These failures compose into a ten-class taxonomy (Appendix~\ref{sec:failure_taxonomy}) traversed by an executable protocol (Algorithm~\ref{alg:decision}), letting a practitioner classify a new system's failure in one pass. The open problem is constructive: designing indexable, order-sensitive comparators that preserve the per-frame signal current aggregators discard.%
\ifdefined\neuripssubmission
\else
