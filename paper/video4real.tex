...
\textbf{Within-match transfer check.} Using a pre-test, canary-frozen 4\,s event window, on
...
detected. Intact DTW exceeds its 10-permutation shuffled-DTW control by $+0.021$
$[0.015, 0.028]$ but has no detected difference from order-free one-to-one assignment
($+0.000$ $[-0.003, 0.004]$); temporal-residual DTW is below BoT ($-0.010$
$[-0.018, -0.002]$). This tests event retrieval with same-match negatives, not cross-match search.
