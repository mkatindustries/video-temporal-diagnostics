...
% Last verified before the SoccerNet transfer-check insertion: seven PDF pages,
% with references beginning on page 6. Rebuild and recheck pagination locally.
%
...
maneuvers \emph{within} a known location. We ask whether this within-location advantage survives
when the search spans many locations. Under a matched query-wise protocol, V-JEPA~2
encoder-sequence DTW beats a bag-of-tokens (BoT) cosine baseline within intersections (Honda HDD
$0.955$ vs.\ $0.923$ mAP; nuScenes $0.922$ vs.\ $0.855$), yet loses over the full evaluation
gallery (HDD $0.177$ vs.\ $0.256$; nuScenes $0.159$ vs.\ $0.333$). Separate order-ablation
controls do not support a general order-specific explanation: intact DTW detectably beats
shuffled DTW only for nuScenes encoder features; order-free assignment differs detectably only
once, improving nuScenes temporal-residual AP.
...
advantage comes from intact order or finer per-frame matching, and whether it survives when the
same query must be found in a much larger gallery (\cref{fig:protocol_schematic}). Its
full-gallery failures are overwhelmingly wrong-location retrievals (\cref{fig:error_composition}).

\begin{figure}[t!]
  \centering
  \includegraphics[width=\linewidth]{v4r_protocol_schematic.png}
  \caption{Same query, different gallery (schematic). Conditional retrieval restricts candidates
  to clips from the query's intersection; global retrieval adds clips from every retained
  intersection. A candidate is relevant only when both its intersection and maneuver match the
  query; intersection identity defines this evaluation relevance but is not given to either video
  scorer (BoT, encoder-sequence DTW). Icons are schematic, not real detections. Bars report the
  aggregate query-macro mAP from \cref{tab:reversal} on a shared $[0,1]$ scale for Honda HDD and
  nuScenes: encoder-sequence DTW leads BoT conditionally but trails it globally.}
...
\textbf{Within-match transfer check.} Using a train-only canary-frozen $4$\,s window, on
$6{,}277$ same-half replay queries from $100$ SoccerNet-v2 test matches~\cite{soccernetv2},
encoder-sequence DTW gives match-macro MRR $0.155$
versus $0.153$ for BoT (paired match-bootstrap $\Delta{=}+0.003$ $[-0.005, 0.010]$), so no gain is
detected. Intact DTW exceeds its shuffled-DTW control by $+0.021$ $[0.015, 0.028]$ but has
no detected difference from the assignment control ($+0.000$ $[-0.003, 0.004]$);
temporal-residual DTW is below BoT ($-0.010$ $[-0.018, -0.002]$). This tests event retrieval
with same-match negatives, not cross-match search.
...
On the two driving tasks, within-intersection and cross-intersection retrieval measure different
...
On an EPIC-Kitchens manifest of $500$ sequences, a separate probe suggests strong interface
dependence in multimodal LLM (MLLM) outputs: three open-weight models have mean balanced
accuracy $0.503$--$0.538$ across three direct direction prompts, whereas INTACT/TAMPERED framing
reaches $0.845$--$0.879$ for Qwen3-VL-8B and Gemma-4-31B; LLaVA-Video's integrity output is constant
and excluded. These descriptive, prompt-specific classification probes do not test full-gallery
retrieval; applying the matched-gallery protocol to MLLM readouts is future work.
...
SoccerNet adds one within-match replay-grounding test split but does not test cross-match search.
