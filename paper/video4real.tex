...
Separately, multimodal LLMs can carry recoverable temporal evidence that standard prompting hides:
on EPIC-Kitchens forward/reverse clips, three direct direction prompts leave Gemma-4, LLaVA-Video,
and Qwen3-VL near chance ($0.503$--$0.538$ balanced accuracy), yet an intact-vs-tampered integrity
prompt reaches $0.845$ (Gemma-4) and $0.879$ (Qwen3-VL) (LLaVA-Video degenerates to constant output;
transcribed from artifacts not in the compact result bundle). A larger multimodal model therefore
does not by itself resolve the retrieval-vs-order tension studied here.
