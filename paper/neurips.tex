...
forward/reverse changes that pooling suppresses, while direct direction prompts stay near
chance and their answer priors shift with phrasing. We consolidate the observed failure
...
\textbf{Evaluation protocols.} We report average precision (AP; area under the precision--recall curve) and, where applicable, area under the ROC curve (AUC). Except for the explicitly identified reranking experiment, these are pooled pair-classification diagnostics, not standard query-wise retrieval metrics. \emph{VCDB}: 528 videos; 5{,}585 positive pairs from annotations, negatives sampled 1:1 with fixed seed (sensitivity analysis across 1:1, 1:5, and all-pairs in Appendix~\ref{sec:neg_sampling}). \emph{HDD}: all $\binom{n}{2}$ pairs within each GPS-clustered intersection (DBSCAN density clustering, $\epsilon\!\approx\!30$\,m); positive = same maneuver; 50 mixed clusters, 1{,}687 maneuvers. \emph{nuScenes}: the same protocol with DBSCAN run independently within each map's local ego\_pose coordinate frame; CAN-bus (Controller Area Network) turn labels; 244 segments, 824 pairs. \emph{EPIC s\textsubscript{rev}}: per-sequence self-comparison on 500 sequences; bootstrap 95\% CIs (1{,}000 resamples). \emph{Nymeria}: 100 egocentric sessions, 8{,}780 segments; positive = same activity same room; all $\binom{n}{2}$ pairs (394{,}210). \emph{MUVR News}: 9{,}958 videos, 74 topics, 27{,}453 positive pairs. \emph{SSv2}: 400 clips across chiral (mirror-direction) template pairs---e.g., ``pushing left to right'' vs.\ ``pushing right to left''---50 per template; within-pair pairwise evaluation where positive = same template and negative = chiral twin.
...
To test whether the ranking transfers, we evaluate on nuScenes~\cite{nuscenes}. Turn labels come from CAN bus, and DBSCAN ($\epsilon\!=\!30$m) runs independently within each map's local coordinate frame, yielding 244 segments across 50 mixed-direction clusters (824 pairs).

V-JEPA~2 encoder-sequence DTW reaches AP\,=\,0.863, ahead of the temporal residual (0.811) and BoT (0.781), while DINOv3 methods are at $\leq$0.614. Paired intersection-cluster bootstrap differences support encoder-sequence DTW over BoT ($+0.082$ [0.028, 0.140]) and over the residual ($+0.052$ [0.033, 0.066]). The residual--BoT contrast is less certain ($+0.030$ [$-0.036$, 0.101]). Differences in native sampling are one possible explanation, not an identified cause.
...
Under these direct prompts, the open-weight models are near chance and Gemini~3.1 Pro~\cite{gemini31pro} reaches $0.599 \pm 0.010$. Near-chance generative accuracy is not by itself evidence that the outputs contain no temporal information: answer priors shift strongly across prompt phrasings (Appendix~\ref{sec:prompt_details}), and sequence comparisons of the same pipelines' internal representations do register forward/reverse changes (Section~\ref{sec:vlm_embedding}). Taken together, these results demonstrate prompt- and readout-dependence. An intact-vs.-tampered integrity probe on the same clips was reported in earlier versions of this paper; it is withdrawn pending a re-run (Appendix~\ref{sec:integrity_probe}) and no conclusion here rests on it.
...
\midrule
DINOv3 BoF & 0.979 & 0.530 & 0.584 & 0.500 & 1.000 \\
DINOv3 Chamfer & \textbf{0.989} & 0.559 & 0.589 & 0.500 & 0.9999 \\
DINOv3 Temporal Deriv. & 0.775 & 0.498 & 0.614 & 0.503 & 0.500 \\
DINOv3 Attn.\ Trajectory & 0.700 & 0.507 & 0.567 & \textbf{0.720} & \textbf{0.192} \\
V-JEPA 2 BoT & 0.838 & 0.825 & 0.781 & 0.555 & 0.984 \\
V-JEPA 2 Encoder-Seq DTW & --- & 0.942 & \textbf{0.863} & --- & --- \\
V-JEPA 2 Temporal Res. & 0.698 & \textbf{0.956} & 0.811 & 0.556 & 0.0033 \\
ViCLIP (InternVid-10M) & 0.907 & 0.542 & --- & --- & 0.996 \\
...
\begin{flushleft}
\small --- = not evaluated; the superseded nuScenes ViCLIP result was removed pending a location-aware rerun. VCDB AP depends on negative seed ($\pm$0.06; Appendix~\ref{sec:neg_sampling}). $s_\text{rev}$ values from cosine and DTW are not cross-comparable; V-JEPA~2 BoT uses cosine and its residual uses sequence DTW.
\end{flushleft}
\end{table}
...
Gemma-4 (SigLIP) & 0.926 & 0.521 & --- & 1.000$^\dagger$ \\
LLaVA (CLIP) & 0.915 & 0.514 & --- & 1.000$^\dagger$ \\
Qwen3-VL (SigLIP) & 0.491 & 0.421 & --- & 0.998$^\dagger$ \\
\midrule
\multicolumn{5}{l}{\emph{Vision tower, per-frame sequence (DTW)}} \\
Gemma-4 (SigLIP) & 0.632 & 0.553 & --- & 0.492 \\
LLaVA (CLIP) & 0.637 & 0.545 & --- & 0.495 \\
Qwen3-VL (SigLIP) & 0.610 & 0.468 & --- & 0.542 \\
\midrule
\multicolumn{5}{l}{\emph{LLM backbone, pooled hidden state}} \\
Gemma-4 & 0.838 & 0.472 & --- & $\geq$0.998 \\
LLaVA & 0.907 & 0.512 & --- & $\geq$0.998 \\
Qwen3-VL & 0.916 & 0.518 & --- & $\geq$0.998 \\
\bottomrule
\end{tabular}
}
\begin{flushleft}
\small $^\dagger$Observed pooled cosine value, clamped to 1.000 where f32 precision exceeded one. Pooled and sequence rows use different comparators and numerical scales. Superseded nuScenes VLM results were removed pending location-aware reruns.
\end{flushleft}
\end{table}
...
\section{VLM Temporal Integrity Probe (Withdrawn)}
\label{sec:integrity_probe}

Separately from forward/reverse classification, we asked VLMs to judge whether a video's temporal structure is ``INTACT'' or ``TAMPERED,'' on forward, reversed, and chunk-scrambled ($K\!=\!4, 16$) versions of EPIC-Kitchens sequences. Earlier versions of this paper tabulated per-condition rates for that probe and concluded that the integrity framing recovered temporal information the direct direction prompts missed. \textbf{We withdraw that result and its table.}

The tabulated per-condition rates are inconsistent with the same run's own recorded cross-transform consistency, i.e.\ the fraction of clips answered correctly under all four conditions. For Qwen3-VL, those rates imply by a union bound over the four conditions a per-clip all-correct rate of at least 8.6\%, whereas the run recorded 3.8\% for that model. All four tabulated columns are exact complements of the corresponding per-condition accuracies plotted by the same run. That is the correct conversion only for the forward condition, where the recorded accuracy is the fraction judged INTACT; for the reversed and scrambled conditions the recorded accuracy is already the fraction judged TAMPERED, so complementing it inverts the value. A uniform ``$1-\text{accuracy}$'' transcription across all four conditions therefore reproduces the published table exactly while inverting three of its four columns. We do not trust the transcribed aggregate, and the balanced accuracies derived from it are not valid. The withdrawn values are retained unchanged in the released results artifact (\texttt{results/epic/vlm\_prompt\_results.json}) alongside a machine-readable record of the defect, so the inconsistency above can be checked.

The per-clip responses from that run were not retained, so the correct rates cannot be recomputed from the stored artifacts. We consequently report no corrected numbers here: the probe can be re-run from the original sequence manifest, and the claim will be restored only if such a re-run supports it. No result in Sections~\ref{sec:vlm_generative}--\ref{sec:vlm_embedding} or in the Discussion depends on this probe.

One observation from the run survives the defect, because it is invariant to the direction of the conversion: LLaVA-Video-7B emits the same integrity label for every input, including under the text-only baseline, so its integrity output is uninformative about the video under either reading.
...
LLaVA-Video produces a constant integrity label regardless of input (text-only baseline confirms prior bias); the integrity probe that used those outputs is withdrawn (Appendix~\ref{sec:integrity_probe}).
...
Table~\ref{tab:bootstrap} reports percentile intervals from 2{,}000 resamples (seed\,=\,42). Corrected nuScenes rows resample intersection clusters; all other rows resample individual pairs and are retained only as descriptive summaries because pairs share clips and other experimental units. Degenerate samples fall back to the point estimate.

\begin{table}[h]
\centering
\caption{Resampling intervals for AP. Rows use descriptive pair resampling except corrected nuScenes, which uses intersection-cluster resampling. ``Width'' is interval high minus low.}
\label{tab:bootstrap}
\begin{tabular}{llccc}
\toprule
...
\midrule
\multirow{7}{*}{nuScenes}
 & V-JEPA 2 Encoder-Seq DTW & 0.863 & [0.826, 0.906] & 0.080 \\
 & V-JEPA 2 Temporal Res.   & 0.811 & [0.773, 0.862] & 0.090 \\
 & V-JEPA 2 BoT             & 0.781 & [0.718, 0.849] & 0.130 \\
 & DINOv3 Temporal Deriv.   & 0.614 & [0.544, 0.709] & 0.166 \\
 & DINOv3 BoF               & 0.584 & [0.536, 0.641] & 0.105 \\
 & DINOv3 Chamfer           & 0.589 & [0.540, 0.645] & 0.105 \\
 & DINOv3 Attn.\ Trajectory & 0.567 & [0.510, 0.647] & 0.136 \\
\midrule
\multirow{6}{*}{Nymeria}
...
\end{tabular}
\begin{flushleft}
\small Pair-level intervals are especially narrow on HDD and Nymeria because many pairs reuse the same clips. On HDD, resampling intersection clusters instead widens the V-JEPA~2 temporal-residual interval from [0.955,\,0.958] to [0.890,\,0.978]. The corrected nuScenes rows already use cluster resampling. Marginal intervals do not test method differences; paired intersection-cluster contrasts for HDD and nuScenes are in Table~\ref{tab:paired_bootstrap}. Analogous independent-unit definitions are needed for VCDB, Nymeria, MUVR, and SSv2 before inferential claims.
\end{flushleft}
\end{table}
...
\midrule
nuScenes & Encoder-Seq DTW $-$ BoT & +0.082 & [0.028, 0.140] \\
nuScenes & Encoder-Seq DTW $-$ Temporal Residual & +0.052 & [0.033, 0.066] \\
nuScenes & Temporal Residual $-$ BoT & +0.030 & [$-0.036$, 0.101] \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:nuscenes_maneuver} visualizes the corrected nuScenes maneuver-discrimination results reported in Section~\ref{sec:nuscenes}. Among the three primary V-JEPA~2 comparators, encoder-sequence DTW (AP\,=\,0.863) leads the temporal residual (0.811) and BoT (0.781), while the order-free encoder assignment control reaches 0.875. DINOv3 methods remain below 0.615. Paired cluster contrasts support encoder-sequence DTW over both primary alternatives; the residual--BoT interval includes zero (Table~\ref{tab:paired_bootstrap}).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{../figures/nuscenes_maneuver_discrimination.png}
    \caption{Location-aware nuScenes pair diagnostic (244 segments, 824 pairs, 50 intersection clusters). Encoder-sequence DTW leads the three primary V-JEPA~2 comparators (AP 0.863), while order-free encoder assignment reaches 0.875; paired primary-method contrasts are in Table~\ref{tab:paired_bootstrap}.}
