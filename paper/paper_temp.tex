...
% Target track: NeurIPS 2026 Evaluations & Datasets (E&D), DOUBLE-BLIND
% (E&D 2026 allows single-blind only for dataset-centered submissions).
% ----- ----------------------------------------------------------------
\ifdefined\neuripssubmission
  \usepackage[eandd,nonatbib]{neurips_2026} % E&D track, double-blind (anonymous)
\else
...
% Author suppressed for double-blind review; restore real author block from git history for camera-ready.
...
The gradient separates method classes (Figure~\ref{fig:scramble_gradient}; Appendix~\ref{sec:scramble_data}). Order-invariant methods stay flat, while attention trajectories degrade most ($-18.3\%$ relative from $K\!=\!1$ to $K\!=\!16$), consistent with gaze paths carrying the most temporal structure.
...
DINOv3 methods fail (AP $\approx$ 0.50) because dashcam attention tracks salient objects identical for both turns. V-JEPA~2 temporal residuals achieve AP\,=\,0.956; a pixel-difference motion proxy rules out a trivial intensity shortcut (Spearman $r = 0.051$; Appendix~\ref{sec:hdd_leakage}), and optical flow (RAFT) achieves only AP\,=\,0.478. Restricting to left-vs-right pairs yields AP\,=\,0.960 [0.958, 0.963].

Encoder-sequence DTW (per-frame patch tokens aligned via DTW instead of mean-pooled) reaches AP\,=\,0.942, so switching pooled-cosine to per-frame DTW alone explains 89\% of the BoT-to-residual gap (0.825$\to$0.956). Cluster-level CIs are 7--32$\times$ wider than pair-level (Appendix~\ref{sec:bootstrap_cis}), and the comparator switch dominates even at the residual's cluster-CI low end ($\geq$0.890). A symmetric-max (Chamfer) aggregation~\cite{kordopatis2019visil} on the same encoder-sequence patches reaches AP\,=\,0.914 without dynamic programming, closing 76\% of the BoT$\to$encoder-sequence DTW gap. The remaining 24\% (0.914$\to$0.942) comes from DTW's monotonic warping constraint. We restrict encoder-sequence DTW to motion tasks (HDD, nuScenes), since on VCDB BoF already hits AP\,=\,0.989.

\textbf{Diagnostic-protocol stress test: does the decomposition compose into a fast retriever?} The decomposition predicts that an indexable BoT pre-filter feeding a DTW reranker should approximate full encoder-sequence DTW at lower compute. We run this on the full 1{,}687-segment corpus (V-JEPA~2 BoT top-$k$ pre-filter, encoder-sequence DTW rerank on survivors, non-survivors scored by a sentinel below the BoT floor; full sweep and RRF alternative in Appendix~\ref{sec:rerank_appendix}). At $k\!=\!100$ the rerank collapses (AP\,=\,0.638) because pre-filter recall is only 0.35. At $k\!=\!1000$ ($\sim\!60\%$ of the corpus) AP reaches 0.934, within 0.008 of full DTW; the two-stage pipeline \emph{is} lossless at sufficient candidate budget. The pre-filter's similarity (appearance) is orthogonal to the task's (maneuver): same-intersection opposite-turn pairs crowd the top-$k$ and evict true positives. The pairwise gap does not compose into a cheap indexable retriever via appearance-based filters. Motion-aware first-stage filtering is the open problem.
...
Full cross-method comparisons across the seven benchmarks are in Appendix~\ref{sec:cross_method_tables} (Tables~\ref{tab:summary_encoders} and~\ref{tab:summary_vlm}). The key finding is that no single method dominates all three regimes. BoF/Chamfer wins scene matching (VCDB AP\,=\,0.989), V-JEPA~2 temporal residual wins motion retrieval (HDD AP\,=\,0.956), and all models---including 31B-parameter VLMs---remain near chance on temporal order classification.
...
\textbf{World model implications.} V-JEPA~2's temporal sensitivity is consistent with its design as a predictive world model~\cite{lecun_worldmodel,vjepa2}. We do not isolate this as the cause, but the sensitivity--invariance trade-off (Figure~\ref{fig:sensitivity_invariance}) is a candidate diagnostic for any temporal-order-dependent representation. Concurrent interpretability~\cite{joseph2026physics} localizes physics representations to an intermediate-depth V-JEPA~2 transition with distributed directional encoding, consistent with temporal sensitivity surviving in the backbone but requiring task-specific comparators to extract.
...
\textbf{Limitations.} DTW is not indexable ($O(T^2)$); probes train on 400 samples in $p \gg n$, bounding recoverability rather than proving absence; ViCLIP's 8-frame input confounds 64-frame V-JEPA~2 comparison; motion-retrieval evidence rests on three benchmarks (HDD, nuScenes, SSv2) with \emph{directional} cross-domain replication.


\section{Conclusion}

We introduced a diagnostic framework localizing where temporal information is preserved or destroyed in video retrieval. The central finding is an unresolved sensitivity--invariance trade-off (Figure~\ref{fig:sensitivity_invariance}). BoF/Chamfer's order-invariance both powers VCDB copy detection and causes motion-direction blindness, and no architecture occupies both regimes. The comparator dominates motion benchmarks (with domain-dependent optimal choice), and reversal detection remains out of reach for all pooled readouts. The protocol also surfaces a negative result, with the indexable BoT$\to$DTW retriever collapsing at $k\!=\!100$ (recall 0.35) and recovering only at $k\!\approx\!1000$. Motion-aware first-stage filtering remains open. The feature-vs-comparator decomposition is modality-agnostic, asking \emph{where} temporal sensitivity resides rather than whether it exists.\footnote{Toolkit code: \url{https://anonymous.4open.science/r/video-temporal-diagnostics-F450}.}
